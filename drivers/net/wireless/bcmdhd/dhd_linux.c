...
//static u32 dhd_concurrent_fw(dhd_pub_t *dhd); //oppo liuhd del for compile
#endif 
...
static int dhd_set_suspend_test(int value, dhd_pub_t *dhd)
{
	int power_mode = PM_MAX;
	/* wl_pkt_filter_enable_t	enable_parm; */
	char iovbuf[32];
...
#if 0
	if(!wake_lock_active(&dhd->wl_wifi_test))
	{
		printk("---bosj --wakelock.\n");
		wake_lock(&dhd->wl_wifi_test);
	}
	#endif
				/* Kernel suspended */
				DHD_ERROR(("%s: force extra Suspend setting \n", __FUNCTION__));
			//	printk("---bosj3333333333333333333333------\n");
				dhd_wl_ioctl_cmd(dhd, WLC_SET_PM, (char *)&power_mode,
				                 sizeof(power_mode), TRUE, 0);
				...
				dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
				mdelay(10);
			//	printk("----bosj---final---run---\n");
				#if 0
		if(wake_lock_active(&dhd->wl_wifi_test))
	{
		printk("---wakeunlock...test...bosj\n");
		wake_unlock(&dhd->wl_wifi_test);
	}
		#endif
...
//static int dhd_suspend_resume_helper_test(struct dhd_info *dhd, int val)
//Bob

static struct dhd_info *dhd_test;
static void dhd_early_suspend(struct early_suspend *h)
{
	 dhd_test = container_of(h, struct dhd_info, early_suspend);
	//struct dhd_info dhd = dhd_test;
	DHD_TRACE(("%s: enter\n", __FUNCTION__));

	//if (dhd)
		dhd_suspend_resume_helper(dhd_test, 1);
}
static int dhd_suspend_resume_helper_test(struct platform_device *pdev, pm_message_t state)

{
	dhd_pub_t *dhdp = &dhd_test->pub;
...
/* Bob */
static struct platform_driver wifi_suspend_driver = {
	.suspend	= dhd_suspend_resume_helper_test,
	//resume		= dhd_suspend_resume_helper,
	.driver = {
		.name = "wifi_suspend",
		.owner = THIS_MODULE,
	},
...
//	BT_INFO("try to register wifi suspend.\n");

	retval = platform_driver_register(&wifi_suspend_driver);
	if (retval)
		printk("wifi register is failed.\n");
 ...
 */
#if 0 //oppo liuhd del for compile
static u32
