...
static s32 wl_inform_single_bss(struct wl_priv *wl, struct wl_bss_info *bi)
{
	...
	//20120718
 //  printk("20120718\n"); 
	// changed by bosj 20120718

	if (bi->ctl_ch > 14 || CHSPEC_CHANNEL(bi->chanspec) > 14)
	{
		printk("**bosj**CHSPEC_CHANNEL(bi->chanspec) = %u, bi->ctl_ch = %u\n", CHSPEC_CHANNEL(bi->chanspec), bi->ctl_ch);
		return err;
	}
...
//	 printk("**bosj******notif_bss_info->channel = %u, bi->ctl_ch = %u, bi->chanspec = %u\n", notif_bss_info->channel, bi->ctl_ch, bi->chanspec);
	
		#if 0
		notif_bss_info->channel =
		(bi->ctl_ch >=1 && bi->ctl_ch <= 14 )? bi->ctl_ch : CHSPEC_CHANNEL(bi->chanspec);
		#endif
		//printk("*bosj222222222******\n");
	...
	WL_DBG(("bosj***SSID : \"%s\", rssi %d, channel %d, capability : 0x04%x, bssid %pM"
