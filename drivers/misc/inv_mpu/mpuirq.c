...
//#define GYRO_SENSOR_PM_GPIO   		PM8058_GPIO_PM_TO_SYS(16)
...
//	res= pm8xxx_gpio_config(gpio_cfgs.gpio,&gpio_cfgs.cfg);
//		if (res < 0) {
//			pr_err("[mpuirq]pmic gpio config failed\n");
//			return res;
//		}
	mpu_irq = gpio_to_irq(mpu_client->irq);
	printk("huyu----------[%s],----------mpu_client->irq = %d , mpu_irq = %d \n", __func__,mpu_client->irq,mpu_irq);
	...
	res = request_threaded_irq(mpu_irq,NULL, mpuirq_handler,
				IRQF_TRIGGER_RISING,
				"MPUIRQ", 0);
