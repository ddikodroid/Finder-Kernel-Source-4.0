...
extern int sensor_power_on(void);
extern int sensor_power_off(void);
...
static int mpu_dev_open(struct inode *inode, struct file *file)
{
	...
	if (mpu->pid) {
		//mutex_unlock(&mpu->mutex);
		//dev_info(&client->adapter->dev, "mpu_dev_open return -EBUSY\n");
		//return -EBUSY;
		if(mpu->pid_test)
		{
			mutex_unlock(&mpu->mutex);
			dev_info(&client->adapter->dev, "mpu_dev_open try to open the third time !! return -EBUSY\n");
			return -EBUSY;
		}
		mpu->pid_test = current->pid;
	}
	else {		
		mpu->pid = current->pid;
	}
	...
	//dev_info(&client->adapter->dev, "mpu_dev_open ----------current->pid %d,mpu->pid = %d,mpu->pid_test = %d \n", current->pid,mpu->pid,mpu->pid_test);
...
static int mpu_release(struct inode *inode, struct file *file)
{
	...
	result = inv_mpu_suspend(mldl_cfg,
				...
				INV_ALL_SENSORS);
	if(current->pid == mpu->pid)
		mpu->pid = 0;
	else if(current->pid == mpu->pid_test)
		mpu->pid_test = 0;
	
	//dev_info(&client->adapter->dev, "mpu_release ----------current->pid %d,mpu->pid = %d,mpu->pid_test = %d \n", current->pid,mpu->pid,mpu->pid_test);
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	res = sensor_power_on();
	if (res < 0) {
		printk("mpu3050_probe  power on Failed\n");
		goto out_register_pm_notifier_failed;
	}
	else
	{
		printk("mpu3050_probe  power on OK\n");
	}
