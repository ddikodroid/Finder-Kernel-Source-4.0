...
/* bootcharge shutdown 2012-06-12*/
/**
 * charge_pm8xxx_reset_pwr_off - switch  PM8058 PMIC chips attached to the system to
 *			  either reset or shutdown when they are turned off
 * @reset: 0 = shudown the PMICs, 1 = shutdown and then restart the PMICs
 *
 * RETURNS: an appropriate -ERRNO error value on error, or zero for success.
 */
int bootcharge_pm8xxx_reset_pwr_off(int reset)
{
	struct pm8xxx_misc_chip *chip;
	unsigned long flags;
	int rc = 0;
	...
	list_for_each_entry(chip, &pm8xxx_misc_chips, link) {
		...
		case PM8XXX_VERSION_8058:
			...
			break;
		case PM8XXX_VERSION_8901:
             /* when bootcharge shutdown ,we only need shutdown pmic8058 and not need to shutdown PM8091*/
			//rc = __pm8901_reset_pwr_off(chip, reset);
			break;
		case PM8XXX_VERSION_8921:
			rc = __pm8921_reset_pwr_off(chip, reset);
...
EXPORT_SYMBOL_GPL(bootcharge_pm8xxx_reset_pwr_off);
/* bootcharge shutdown 2012-06-12*/
