...
#define CAPACITY_SALTATE_COUNTER_LOW 6 
...
static int bq27541_id_check(struct bq27541_device_info *di)
{
	...
	if(di){
		for(i = 0; i < 5; i++)
		{
			...
			if(ret >=0){
				if(temp == BQ27541_CHEM_ID || temp == BQ27541_CHEM_ID_2){
					gague_counter ++;
...
/*
*   Add bq27541 gauge chip function to use in msm_charger.c
*/
static int get_battery_voltage_fuelgague(void)
{
	...
	if(fuelgague_is_present()){
		batt_vol = bq27541_get_battery_voltage();
	      ...
	      batt_vol = adc_get_battery_mvolts();
	}
...
static int get_battery_temperature_fuelgague(void)
{
	...
	if(fuelgague_is_present()){
		...
		if(vbatt_temp > BATTERY_SHUTDOWN_TEMP){
	       ...
	       vbatt_temp = BATTERY_DEFAULT_TEMP;
	}

	return vbatt_temp;
}

static int get_ichg_currernt_fuelgague(void)
{
     	int ichg_mvolts;
	...
	if(fuelgague_is_present()){
		ichg_mvolts = bq27541_get_ichg_current();
	       ...
	       ichg_mvolts = adc_get_chg_current();
	}
...
static struct oppo_battery_fuelgauge bq27541_batt_fuelgauge = {
	...
	.get_vchg_mvolts = get_vchg_voltage_fuelgague,
	.get_ichg_current = get_ichg_currernt_fuelgague,
				 ...
				 const struct i2c_device_id *id)
{
	...
	smb346_battery_fuelgauge_register(&bq27541_batt_fuelgauge);

    pr_info("%s:update vchg mvolts boot stegs\n",__func__);	
	smb346_vchg_mvolts_update();
	return 0;
...
static int bq27541_battery_remove(struct i2c_client *client)
{
	...
	/*unregister batt gauge to msm charger*/
       smb346_battery_fuelgauge_unregister(&bq27541_batt_fuelgauge);
	return 0;
}
...
static int bq27541_battery_i2c_resume(struct i2c_client *client)
{
	...
	updata_batt_params_fulegague_resume();
