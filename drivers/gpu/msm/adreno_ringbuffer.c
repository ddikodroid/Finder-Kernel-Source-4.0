...
int adreno_ringbuffer_start(struct adreno_ringbuffer *rb, unsigned int init_ram)
{
	...
	*/
	...
	adreno_regwrite(device, REG_SCRATCH_UMSK,
        ...
        /* update the eoptimestamp field with the last retired timestamp *///liuyan 2012.8.14
        kgsl_sharedmem_writel(&device->memstore,
                            KGSL_DEVICE_MEMSTORE_OFFSET(eoptimestamp),
                            rb->timestamp);
        //liuyan 2012.8.14 end
				...
				unsigned int flags)
{
	...
	//link = kzalloc(sizeof(unsigned int) * (numibs * 3 + 2), GFP_KERNEL);//liuyan 2012.8.14 kgsl patch
	cmds = link;
	if (!link) {
	...
	adreno_drawctxt_switch(adreno_dev, drawctxt, flags);
     //  *cmds++ = cp_type3_packet(CP_WAIT_FOR_IDLE, 1);//liuyan2012.8.14
	// *cmds++ = 0;                                                               //liuyan2012.8.14 kgsl patch
