...
#define WAKE_UNLOCK_DELAY	msecs_to_jiffies(2000)

static void msm_otg_unwake_w(struct work_struct *w)
{
	struct msm_otg *dev = container_of(w, struct msm_otg, wake_unlock_work.work);
...
#endif
		wake_lock(&dev->wlock);
		cancel_delayed_work_sync(&dev->wake_unlock_work);
		queue_work(dev->wq, &dev->sm_work);
	}
out:
	return ret;
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&dev->wake_unlock_work, msm_otg_unwake_w);
