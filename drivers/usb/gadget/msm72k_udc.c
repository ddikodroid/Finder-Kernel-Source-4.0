...
//add by chendx reason NON CHARGER DETECT 2011-10-08
...
struct completion chg_detect_wait;//add by chendx 2011-10-11
...
#define USB_FLAG_CONFIGURED     0x0020
 ...
 //modify by chendx 2011-09-01 Modify begin for detect using PHY
#define USB_CHG_DET_DELAY	msecs_to_jiffies(1000)
#else
#define USB_CHG_DET_DELAY	msecs_to_jiffies(200)
#define USB_CHG_DET_POLL_DELAY	msecs_to_jiffies(40)//40ms
...
#define DETECT_NUM_OF_ATTEMPTS 3

#ifdef NON_STANDARD_CHARGER_SUPPORT
#define PORTSC_LS_NON_STANDARD	(2 << 10)
#endif /* NON_STANDARD_CHARGER_SUPPORT */
...
struct usb_info {
	...
	*/
	...
	struct delayed_work chg_startup_det;
...
/* OPPO 2011-01-14 wangjc Add begin for detect using PHY. */
	unsigned attempts_remaining;
/* OPPO 2011-01-14 wangjc Add end */
};
...
{
#ifdef CONFIG_OPPO_MODIFY
 //modify by chendx 2011-09-01 
/* OPPO 2010-10-08 wangjc Modify begin for add non-standard charger */
#ifndef NON_STANDARD_CHARGER_SUPPORT
	if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS)
		return USB_CHG_TYPE__WALLCHARGER;
	else
		return USB_CHG_TYPE__SDP;
#else
	int portsc_ls = (readl(USB_PORTSC) & PORTSC_LS);

	if (portsc_ls == PORTSC_LS)
		return USB_CHG_TYPE__WALLCHARGER;
	else if(portsc_ls == PORTSC_LS_NON_STANDARD)
		return USB_CHG_TYPE__NON_STANDARD;
	else
		return USB_CHG_TYPE__SDP;
#endif /* NON_STANDARD_CHARGER_SUPPORT */
#else /*CONFIG_OPPO_MODIFY*/
	if ((readl(USB_PORTSC) & PORTSC_LS) == PORTSC_LS)
		return USB_CHG_TYPE__WALLCHARGER;
	else
		return USB_CHG_TYPE__SDP;
#endif
/* OPPO 2010-10-08 wangjc Modify end */
}

//porting by chendx from 11089 project 2011-09-01
/* OPPO 2010-10-08 wangjc Add begin for add non-standard charger */
#ifndef NON_STANDARD_CHARGER_SUPPORT
#define USB_WALLCHARGER_CHG_CURRENT 1800
#else
#define USB_WALLCHARGER_CHG_CURRENT 650
#define NON_STANDARD_CHG_CURRENT 450
#endif /* NON_STANDARD_CHARGER_SUPPORT */
...
//porting by chendx from 11089 project 2011-09-01
/* OPPO 2010-10-08 wangjc Add begin for add non-standard charger support */
#ifdef NON_STANDARD_CHARGER_SUPPORT
	if (temp == USB_CHG_TYPE__NON_STANDARD)
		return NON_STANDARD_CHG_CURRENT;
#endif /* NON_STANDARD_CHARGER_SUPPORT */
/* OPPO 2010-10-08 wangjc Add end */

	if(temp == USB_CHG_TYPE__HDMI)
		return NON_STANDARD_CHG_CURRENT;
...
//ftm mode diag disconnect 2012-06-13
...
/* OPPO 2011-06-08 wangjc Modify begin for notify charger remove and release wakelock. */
#if 0
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	enum chg_type temp;
  ...
  //ftm mode diag disconnect 2012-06-13  
	if (factory_mode == 0){
		wake_lock_timeout(&ui->wlock, HZ);//deleted by songxh 
	}else{
		wake_unlock(&ui->wlock);//added by songxh 
	}
	//ftm mode diag disconnect 2012-06-13
#endif
/* OPPO 2011-06-08 wangjc Modify end */
}

#ifdef CONFIG_OPPO_MODIFY
 //porting from 10089 project by chendx 2011-09-01 
/**
 * OPPO 2011-03-31 wangjc Add
 * DESCRIPTION: - cancel wall charger detection.
 * @param: usb_info.
 * @return: void.
 */ 
/*add by chendx 2011-09-26 for non-charger detect dn on or off*/
#define PM8058_GPIO33	PM8058_GPIO_PM_TO_SYS(32) //use pmic8058 gpio33 to non-chg detect 3.3v
void pmic8058_gpio33_dn_on(int gpio,int gpio_value)
{
...
/*end by chendx 2011-09-26 for non-charger detect dn on or off*/
static void wall_charger_cancel_detection(struct usb_info *ui)
{
	if(ui->attempts_remaining != 0)
	{
		/* wait incase chg_detect is running */
		if (!ui->gadget.is_a_peripheral)
			cancel_delayed_work_sync(&ui->chg_det);

		ui->attempts_remaining = 0;
	}
}

/**
 * OPPO 2011-03-31 wangjc Add
 * DESCRIPTION: - indicate DP is pulled up or not.
 * @param: usb_gadget.
 * @return: bool.
 */ 
static bool msm72k_is_pullup_on(struct usb_gadget *_gadget)
{
	struct usb_info *ui = container_of(_gadget, struct usb_info, gadget);
	return (0 != (readl(USB_USBCMD) & USBCMD_RS));
}

/**
 * OPPO 2011-03-31 wangjc Add
 * DESCRIPTION: - detect usb charger type using phy.
 * @param: usb_info,dm status.
 * @return: is dedicated charger or not.
 */ 
static int usb_chg_detect_using_phy(struct usb_info *ui, bool *is_dm_on)
{
#if  1//2012-5-21 jaingsm modify begin for charger type detecting
	u8 reg_chg_detect = 0;  
	u8 reg_func_ctrl = 0;
	bool is_dedicated_charger = true;
	u8 reg_ulpi_debug=0;

  	/* Read current value of function control  register: */
	reg_func_ctrl = ulpi_read(ui, ULPI_FNCTL);
	if(reg_func_ctrl == 0xffffffff)
	{
		pr_err("Reading from PHY failed. Assuming no charger.\n");
		return -ENODEV;
	}

     pr_debug("%s:reg_func_ctrl:0x%x\n", __func__,reg_func_ctrl);
	/* Check if the PHY is already in the non-drive mode */
	if ((reg_func_ctrl & PHY_FNCTL_MASK_MODE) != PHY_FNCTL_MASK_MODE_NON_DRIVE)
	{
		 /* Set PHY to non-driving mode for charger detection */      
		 ulpi_write(ui, (reg_func_ctrl & ~PHY_FNCTL_MASK_MODE)
				| PHY_FNCTL_MASK_MODE_NON_DRIVE, ULPI_FNCTL);
	}

	/* Read current value of the charger detect. and calib. register */
	reg_chg_detect = ulpi_read(ui, ULPI_CHG_DETECT_REG);
       pr_debug("%s:reg_func_ctrl#1:0x%x\n", __func__,reg_chg_detect);
	/* Clear the write registers, as they will be overwritten */
	reg_chg_detect &= ~PHY_CHGDET_MASK_WRITE; 

	/* Configure charger detection to be via ULPI and not external pins,
	* turn off power, and disabled charger detection 
	*/
	ulpi_write(ui, reg_chg_detect
		| PHY_CHGDET_MASK_CHGRMODE
		| PHY_CHGDET_MASK_CHGRDETON_M
		| PHY_CHGDET_MASK_CHGRDETEN_M, ULPI_CHG_DETECT_REG);

	if(ui->attempts_remaining == 1)
	{
	       /*pmic8058 gpio33*/
		pmic8058_gpio33_dn_on(PM8058_GPIO33,1);
		//msm_dn_on_off(1);
		udelay(500);
	}

	reg_ulpi_debug = ulpi_read(ui, ULPI_DEBUG);
	pr_debug("%s:reg_ulpi_debug:0x%x\n", __func__,reg_ulpi_debug);

	if(reg_ulpi_debug & 0x3)
	{
		*is_dm_on = true;
		//pr_info("is_dm_on:%d\n", *is_dm_on);
	}

	/* Make sure bit (chgrdetonM) is unset to supply power for charger 
	* detection  
	*/
	ulpi_write(ui, reg_chg_detect
		| PHY_CHGDET_MASK_CHGRMODE
	...
	/* There need to be 1ms delay between (chrdetonM) and (chgrdetenM) */
	udelay(100);

	/* Make sure both (chgrdetenM) and (chgrdetenM) are unset to enable 
	* the wall charger detection.
	*/
	ulpi_write(ui, reg_chg_detect
		| PHY_CHGDET_MASK_CHGRMODE, ULPI_CHG_DETECT_REG);

	/* Wait to make sure that (chgrdetenM) is unset and output is ready */
	udelay(100);	

	reg_chg_detect = ulpi_read(ui, ULPI_CHG_DETECT_REG);
	pr_debug("%s:reg_chg_detect#2:0x%x\n",__func__, reg_chg_detect);

	if(ui->attempts_remaining == 1){
		pmic8058_gpio33_dn_on(PM8058_GPIO33,0);
		//msm_dn_on_off(0);
	}
		
	//is_dedicated_charger = ((reg_chg_detect & PHY_CHGDET_MASK_VOUT) != 0);
	if(reg_chg_detect & PHY_CHGDET_MASK_VOUT)
		is_dedicated_charger = true;
	else
		is_dedicated_charger = false;
	pr_debug("chendx:test is_dedicated_charger:%d\n", is_dedicated_charger);

	/* After detection is completed, make sure charger detection is disabled 
	* and not controlled via ULPI
	*/
	reg_chg_detect &= PHY_CHGDET_MASK_WRITE; 

	/* Make sure charger detect is disabled (not controlled via ULPI) */
	ulpi_write(ui, reg_chg_detect
		| PHY_CHGDET_MASK_EXTCHGRCTRL
		| PHY_CHGDET_MASK_CHGRMODE
		| PHY_CHGDET_MASK_CHGRDETON_M
		| PHY_CHGDET_MASK_CHGRDETEN_M, ULPI_CHG_DETECT_REG);

	/* Check if the previous value was non-drive */
	if ((reg_func_ctrl & PHY_FNCTL_MASK_MODE) != PHY_FNCTL_MASK_MODE_NON_DRIVE)
	{
		/* Restore previous setting */
		ulpi_write(ui, reg_func_ctrl, ULPI_FNCTL);
	}
#else
	u8 reg_chg_detect = 0;  
	u8 reg_chg_detect_bak = 0;
	u8 reg_func_ctrl = 0;
	bool is_dedicated_charger = true;
	u8 reg_ulpi_debug=0;
	...
	/* Read current value of the charger detect. and calib. register */
	reg_chg_detect = ulpi_read(ui, ULPI_CHG_DETECT_REG);
	reg_chg_detect_bak = reg_chg_detect;
       pr_err("%s:reg_chg_detect default:0x%x\n", __func__,reg_chg_detect);

	/* set charger detection to be via ULPI :set this bit to zero for ULPI,set this bit to one for ext(default)*/
	reg_chg_detect &=~PHY_CHGDET_MASK_EXTCHGRCTRL;
	/* set charger detect mode to device*/
	reg_chg_detect |=  PHY_CHGDET_MASK_CHGRMODE;
	/* enable charger detect power on control*/
	reg_chg_detect &= ~PHY_CHGDET_MASK_CHGRDETON_M;
	ulpi_write(ui, reg_chg_detect, ULPI_CHG_DETECT_REG);
	dsb();
	udelay(100);	
	/* There need to be at least 20us delay between (chrdetonM) and (chgrdetenM) */
	/*enable charger detect, */
	reg_chg_detect &= ~PHY_CHGDET_MASK_CHGRDETEN_M;
	ulpi_write(ui, reg_chg_detect, ULPI_CHG_DETECT_REG);
	dsb();
	udelay(100);
	
	reg_chg_detect = ulpi_read(ui, ULPI_CHG_DETECT_REG);
	pr_err("%s:reg_chg_detect using ulpi:0x%x\n",__func__, reg_chg_detect);
	...
	pr_err("is_dedicated_charger:%d\n", is_dedicated_charger);
		
	/* After detection is completed, make sure charger detection is disabled 
	* and not controlled via ULPI
	*/
	/* Make sure charger detect is disabled (not controlled via ULPI) */
	ulpi_write(ui, reg_chg_detect_bak, ULPI_CHG_DETECT_REG);
	dsb();
	if (is_dedicated_charger == false){
	       /*pmic8058 gpio33*/
		pmic8058_gpio33_dn_on(PM8058_GPIO33,1);
		dsb();
		udelay(500);
...
#endif//2012-5-21 jiangsm modify end

	return is_dedicated_charger;
}
#endif 

#ifndef CONFIG_OPPO_MODIFY
 //porting from 10089 project by chendx for detect charger using PHY. 2011-09-01 
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	unsigned long flags;
	int maxpower;
	bool is_dedicated_charger = false;
	bool is_dm_on = false;

    //pr_info("usb_chg_detect ready now!!!!\n");

	
	if(ui->attempts_remaining == 0)
	{
		pr_info("%s invalid attempts\n", __func__);
		return;
	}
	
	if(msm72k_is_pullup_on(&ui->gadget) == true)
	{
		pr_err("D+ is already pull up.\n");
		//return;
	}
	else
	{
		is_dedicated_charger = usb_chg_detect_using_phy(ui, &is_dm_on);
	}

	pr_debug("usb_chg_detect @_@ is_dedicated_charger = %d, is_dm_on %d \n",is_dedicated_charger,is_dm_on);
	
	if(is_dedicated_charger == true)
	{
		spin_lock_irqsave(&ui->lock, flags);
		if (ui->usb_state == USB_STATE_NOTATTACHED) {
			spin_unlock_irqrestore(&ui->lock, flags);
	...
	else
	{
		...
		if(is_dm_on == true){
			temp = USB_CHG_TYPE__NON_STANDARD;
		}
		else
		...
		else
		{
			schedule_delayed_work(
							&ui->chg_det,
							USB_CHG_DET_POLL_DELAY);
		}
	}
}
#endif

static void usb_chg_delay_detect(struct work_struct *w)
{
	struct usb_info *ui = container_of(w, struct usb_info, chg_startup_det.work);
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	int maxpower;
...
static irqreturn_t usb_interrupt(int irq, void *data)
{
	...
	if (n & STS_URI) {
		...
		msm_hsusb_set_state(USB_STATE_DEFAULT);
		atomic_set(&ui->remote_wakeup, 0);
/* OPPO 2011-05-16 wangjc Delete begin for don't stop charging in reset. */
#if 0
		if (!ui->gadget.is_a_peripheral)
			schedule_delayed_work(&ui->chg_stop, 0);
#endif
/* OPPO 2011-05-16 wangjc Delete end */
...
static void usb_prepare(struct usb_info *ui)
{
	...
	INIT_DELAYED_WORK(&ui->chg_startup_det, usb_chg_delay_detect);
...
/* OPPO 2011-03-31 wangjc Modify begin for detect charger using PHY.  */
#ifndef CONFIG_OPPO_MODIFY
 //porting from 10089 project by chendx 2011-09-01 
				msm72k_pullup_internal(&ui->gadget, 1);
#else
				wall_charger_cancel_detection(ui);
				ui->attempts_remaining = DETECT_NUM_OF_ATTEMPTS;
#endif
/* OPPO 2011-03-31 wangjc Modify end */
...
/* OPPO 2011-06-08 wangjc Modify begin for release wakelock later. */
#if 0
				wake_unlock(&ui->wlock);
#else
				schedule_delayed_work(&ui->chg_stop, 20);
#endif
/* OPPO 2011-06-08 wangjc Modify end */
				break;
			}
...
/* OPPO 2011-05-16 wangjc Delete begin for don't stop charging in suspend. */
#if 0
				otg_set_power(ui->xceiv, 0);
#endif
/* OPPO 2011-05-16 wangjc Delete end */
...
/* OPPO 2010-10-08 wangjc Modify begin for add non-standard charger support */
#ifndef NON_STANDARD_CHARGER_SUPPORT
	if (is_active || atomic_read(&otg->chg_type)
					 == USB_CHG_TYPE__WALLCHARGER)
#else
	if (is_active || atomic_read(&otg->chg_type) == USB_CHG_TYPE__WALLCHARGER || 
			atomic_read(&otg->chg_type) == USB_CHG_TYPE__NON_STANDARD||
			atomic_read(&otg->chg_type) == USB_CHG_TYPE__HDMI)
#endif /* NON_STANDARD_CHARGER_SUPPORT */
/* OPPO 2010-10-08 wangjc Modify end */
		wake_lock(&ui->wlock);
...
static int msm72k_pullup(struct usb_gadget *_gadget, int is_active)
{
	...
	pr_debug("msm72k_pullup @_@ test_flag = %d, is_active = %d \n",startup_flag,is_active);
...
#if 0
  /*oppo, 2011-3-19, likun comments it to fix usb mode switch failed, start*/
    /*开机过程强制枚举3次*/
    if (is_active && !ui->gadget.is_a_peripheral)
	schedule_delayed_work(&ui->chg_det, USB_CHG_DET_DELAY);

    /*oppo, 2011-3-19, likun comments it to fix usb mode switch failed, end*/
#endif
	return 0;
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	char *chg_type[] = {"STD DOWNSTREAM PORT",
			"CARKIT",
			"DEDICATED CHARGER",
                     /* OPPO 2010-10-08 wangjc Add begin for define non-standard charger */
...
/*delay detect charger type*/
struct hrtimer chg_detect_timer;
#define CHG_DETECT_DELAY 10
static enum hrtimer_restart chg_detect_delay__func(struct hrtimer *timer)
{	
	struct usb_info *ui = the_usb_info;
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
	
	pr_info("%s: delay detect\n",__func__);

	/*check usb if enumerate ,if enumerate we will set the charger type to USB,otherwise change to PHY detect
	charger type*/
	if(ui->usb_state == USB_STATE_CONFIGURED)	{
		pr_info("%s: ###USB PORT CONNECT\n",__func__);
		schedule_delayed_work(&ui->chg_startup_det, USB_CHG_DET_DELAY);
	}else if(ui->usb_state == USB_STATE_NOTATTACHED) {
		return HRTIMER_NORESTART;
	}else{
		pr_info("%s: ###USB DISCONNECT\n",__func__);
		/*chg det force to detect by PHY when not USB_CHG_TYPE__SDP */
		if (atomic_read(&otg->chg_type) != USB_CHG_TYPE__WALLCHARGER && !ui->gadget.is_a_peripheral){
			wall_charger_cancel_detection(ui);
			ui->attempts_remaining = DETECT_NUM_OF_ATTEMPTS;
			/*pull down D+*/
			writel(readl(USB_USBCMD) & ~USBCMD_RS, USB_USBCMD);
...
void chg_delay_detect_init_timer(void)
{
	hrtimer_init(&chg_detect_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	chg_detect_timer.function = chg_detect_delay__func;
	hrtimer_start(&chg_detect_timer, ktime_set(CHG_DETECT_DELAY, 0), HRTIMER_MODE_REL); 
...
static int msm72k_probe(struct platform_device *pdev)
{
   ...
   /*add by chendx reason chg_detect_wait init 2011-10-11*/
      init_completion(&chg_detect_wait);
   /*end by chendx reason chg_detect_wait init 2011-10-11*/
	...
	/*delay detect charger type 2012-06-12 */
	chg_delay_detect_init_timer();
	/*delay detect charger type 2012-06-12 */
