...
extern struct mhl_state_desc mhl_state;
...
#if 0 //commented by zhangyue for it's not using
...
void SiiMhlTxInitialize( )
{
	...
	//modified by zhangyue on 2011-11-22 for concentrating the code
	#ifdef mhl_power_dbg
      mhl_state.initInt = 0;
	#else
	mhlTxConfig.initInt = 0; //indicate the Init INT has not come
	#endif
	//modified by zhangyue on 2011-11-22 end
	#endif
...
// SiiMhlTxClrDCapRdy
//
#if 0 //commented by zhangy for not used
static bool_t SiiMhlTxClrDCapRdy( void )
{
	mhlTxConfig.connectedReady &= ~MHL_STATUS_DCAP_RDY;  // update local copy
	return SiiMhlTxSetStatus( MHL_STATUS_REG_CONNECTED_RDY, mhlTxConfig.connectedReady);
...
{
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2011-12-15 for solving some hpd problems
	//modified by zhangyue on 2011-12-15 for debug audio problems start
	//modified by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem start
	mhl_state.mhl_cnt_st = connected;
	if(mhl_state.mhl_cnt_st && mhl_state.irq_Inited){
		external_common_enable_hpd_feature(connected);
	}
	//if(connectedconnected){
	//    external_common_enable_hpd_feature(connected);
	//}
	
	//modified by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem end
	//external_common_enable_hpd_feature(connected);
	//modified by zhangyue on 2011-12-15 for debug audio problems end
	#else
