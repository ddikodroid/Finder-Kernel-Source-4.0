...
//add for MHL stanard charge
#include <linux/smb346-charger.h>
//end for MHL stanard charge
...
//
//modified  by zhangyue for solving a resuming problem(results the machine down) for debugging on 2011-12-17 start
uint8_t	fwPowerState = POWER_STATE_FIRST_INIT;
//static	uint8_t	fwPowerState = POWER_STATE_FIRST_INIT;
//modified by zhangyue for solving a resuming problem(results the machine down) for debugging on 2011-12-17 end
...
#ifdef ZY_MHL_CHARGE
extern struct completion chg_detect_wait;
#endif
...
bool_t SiiMhlTxChipInitialize (void)
{
    ...
    //deleted by zhanyue on 2011-12-16 for debugging the abnormal init irq
...
static void WriteInitialRegisterValues (void)
{
    ...
    SiiRegWrite(REG_DISC_CTRL1, 0x27);				// Enable CBUS Device discovery
...
static void ForceUsbIdSwitchOpen (void)
{
    ...
    SiiRegWrite(REG_DISC_CTRL3, 0x86);					//CBUS Reset watchdog select 64ms; CBUS communication immediately after MHL discovery.
...
void ProcessRgnd (void)
{
    ...
    if (0x02 == rgndImpedance){
        ...
        #endif
        #ifdef ZY_MHL_CHARGE
	  //charger_comp should be defined and initialized by Chendx, Here we don't know the MHL device type(MHL sink or MHL Dongle) 
	  //modified by zhengzk on 2012.4.11 to pass compile
	  complete(&chg_detect_wait);  
	  //MHL device notify charge module mhl plugin 	
	  //modified by zhengzk on 2012.4.11 to pass compile
	  //mhl_present_notify();
	  #endif        
...
void SwitchToD0 (void)
{
    ...
    SiiRegWrite(REG_DISC_CTRL1, 0x25);	
...
static int Int4Isr (void)
{
    ...
    //0x95 is the value when mhl in D3 mode, and the interrupt is wrong!
    #ifdef CONFIG_OPPO_MODIFY
    //modified by zhangyue on 2011-11-17 for tackking wrong interrupt	
    ...
    }else if(int4Status & BIT3){// process USB_EST interrupt	
...
static void MhlTxDrvProcessConnection (void)
{
    ...
    #ifdef CONFIG_OPPO_MODIFY
    //added by zhangyue for add our Dongle match code on 2011-12-12 start
    // Set Dongle 9292 GPIO0 to low to open PMOS on VBUS
    SiiRegWrite(TX_PAGE_CBUS | 0x13, 0x33); // enable backdoor access
    SiiRegWrite(TX_PAGE_CBUS | 0x14, 0x80); 
    SiiRegWrite(TX_PAGE_CBUS | 0x12, 0x08); 
    
    // Set GPIO0=low
    SiiRegWrite(TX_PAGE_CBUS | 0xc0, 0xff); // main page ; FE for Cbus page
    SiiRegWrite(TX_PAGE_CBUS | 0xc1, 0x7F); // offset
    SiiRegWrite(TX_PAGE_CBUS | 0xc2, 0xFC); // data set GPIO0=Lo ; 0xF3 set GPIO1=Lo
    SiiRegWrite(TX_PAGE_CBUS | 0x20, 0x02);
    SiiRegWrite(TX_PAGE_CBUS | 0x13, 0x48);
    SiiRegWrite(TX_PAGE_CBUS | 0x12, 0x10);
    
    // Set GPIO0=Hi
    //SiiRegWrite(TX_PAGE_CBUS | 0xc0, 0xff); // main page ; FE for Cbus page
    //SiiRegWrite(TX_PAGE_CBUS | 0xc1, 0x7F); // offset
    //SiiRegWrite(TX_PAGE_CBUS | 0xc2, 0xFE); // data set GPIO0=Hi ; 0xFB set GPIO1=Hi
    //SiiRegWrite(TX_PAGE_CBUS | 0x20, 0x02); // burst length?\1
    //SiiRegWrite(TX_PAGE_CBUS | 0x13, 0x48); // offset in scratch pad
    //SiiRegWrite(TX_PAGE_CBUS | 0x12, 0x10); // trig this command
    
    SiiRegWrite(TX_PAGE_CBUS | 0x13, 0x33); // disable backdoor access
    SiiRegWrite(TX_PAGE_CBUS | 0x14, 0x00);
    SiiRegWrite(TX_PAGE_CBUS | 0x12, 0x08);
    //added by zhangyue for add our Dongle match code on 2011-12-12 end
    #endif
...
static void MhlTxDrvProcessDisconnection (void)
{
    ...
    SwitchToD3();
	//added by zhangyue on 2012-05-03 for making USB disconnect from mhl
	switch_to_mhl(0);
}
...
void CbusReset (void)
{
    ...
    UNMASK_INTR_4_INTERRUPTS;
    #ifdef CONFIG_OPPO_MODIFY
    //modified by zhanyue on 2011-12-16 for debugging the abnormal init irq
...
static void MhlCbusIsr (void)
{
    ...
    if( cbusInt ){
        ...
        TX_DEBUG_PRINT(("Drv: CBUS INTR_2: 0x%02x\n", (int) cbusInt));
    }
    ...
    //temply solution, the original expression is  if(cbusInt & BIT3)
    //if(cbusInt & BIT3 || cbusInt & BIT2) {
    {
