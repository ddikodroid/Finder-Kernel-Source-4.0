...
#define zy_cdx_merged 1
#ifdef  zy_cdx_merged
//add by zhangyue
//modified by zhengzk on 2012.4.11 to pass compile
//#include <linux/max8903a-charger.h>
#endif
...
//added by zhangyue on 2011-12-07 for debug the MHL IRQ problem
#define zy_dbg_irq	1
//added by zhangyue on 2011-12-14 for adding the RCP implementation start
#include <linux/input.h>
//You can search the macro ZY_IMPLEMENT_RCP to know all the code added or modified for RCP implementation
#define ZY_IMPLEMENT_RCP	1
//add some code for err handling if err happend in probe the mhl device.
#define zy_dbg_err_handle     1
//added by zhangyue on 2011-12-14 for adding the RCP implementation end
#endif

typedef _Bool bool_t;

//added by zhangyue on 2011-11-22 for adding the mhl suspend function
struct mhl_state_desc{
	bool state;
      //added by zhangyue on 2011-11-22 for concentrating the modified code
    ...
    #endif
    //deleted by zhangyue on 2011-12-16 as Silicon had solved the first abnormal Interrrupt start
    //added by zhangyue on 2011-11-22  end
    //added by zhangyue on 2011-12-01 for debug start
    #ifdef zy_usrspace_enalbe_mhl_irq
    //true--indicate the mhl irq is initialized and enabled, fasle indicate the irq was not enabled.	
    //can write only one time from userspace. here from external_common.c
    bool irq_Inited; 
    #endif  
    //added by zhangyue on 2011-12-01 for debug end
    #ifdef ZY_IMPLEMENT_RCP
    struct input_dev * rmt_input;
    #endif
	//added by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem start
	bool mhl_cnt_st;
	//added by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem end
};
//added by zhangyue on 2011-11-22 end
