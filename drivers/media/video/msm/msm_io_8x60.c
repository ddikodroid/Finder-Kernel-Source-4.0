...
/* Modified by yxq 2012-04-16 start */
...
#endif
/* Modified by yxq 2012-04-16 end */
...
static void msm_subcamera_vreg_enable(void)
{	
	ldo14 = regulator_get(NULL, "8901_l4");
	if (IS_ERR(ldo14)) {
		pr_err("%s: VREG ldo14 get failed\n", __func__);
		ldo14 = NULL;
		return;
	}
	if (regulator_set_voltage(ldo14, 2850000, 2850000)) {
		pr_err("%s: VREG ldo14 set voltage failed\n",  __func__);
		goto ldo14_disable;
	}
	if (regulator_enable(ldo14)) {
		pr_err("%s: VREG ldo14 enable failed\n", __func__);
		goto ldo14_put;
	}
	else {
	pr_err("%s: VREG ldo14 is OK!\n", __func__);
	}
	msleep(2);
...
ldo1_put:
	regulator_put(ldo1);
ldo14_disable:
	regulator_disable(ldo14);
ldo14_put:
	regulator_put(ldo14);
}

static void msm_subcamera_vreg_disable(void)
{
	if (lvs2) {
		regulator_disable(lvs2);
		regulator_put(lvs2);
...
void msm_camera_vreg_ctl(int enable, int is_sub_cam)
{
	if(is_sub_cam) {
		if(enable)
			msm_subcamera_vreg_enable();
		else
			msm_subcamera_vreg_disable();
	} else {
		if(enable)
...
/*OPPO sunjianbo 2012-2-1 ESD*/
void msm_camio_camif_pad_reset_csi(int restore)
{
	uint32_t val;
	if(restore) {		
...
/* Modified by yxq 2012-04-16 start */
	...
	if (camdev->is_subcamera == 1) {
		...
		msleep(1);
		if(camdev->camera_hw_reset) {
			camdev->camera_hw_reset();
		}
		return rc;
	} else {
		msm_camera_vreg_enable();
