...
static int vfe31_config_axi(int mode, struct axidata *ad, uint32_t *ao)
{
	...
	vfe31_reset_free_buf_queue_all();/*OPPO ESD*/
...
static int vfe31_proc_general(struct msm_vfe31_cmd *cmd)
{
	...
	case V31_ROLL_OFF_CFG: {
		...
		cmdp_local, 16);
		cmdp_local += 4;
		vfe31_ctrl->rolloff_update = 0x07;
...
static void vfe31_process_output_path_irq_0(uint32_t ping_pong)
{
	...
	*/
	if ((VFE_MODE_OF_OPERATION_ZSL == vfe31_ctrl->operation_mode ||
		VFE_MODE_OF_OPERATION_VIDEO == vfe31_ctrl->operation_mode) &&/* OPPO 2011-11-23 leihui Add begin for reason */
		(vfe31_ctrl->rolloff_update & 0x01)) {
		vfe31_ctrl->rolloff_update &= (~0x01);
		skip_frame = true;
	} else {	
...
static void vfe31_process_zsl_frame(uint32_t ping_pong)
{
	...
	/* OPPO 2012-01-03 tanrf Add begin */
	if (vfe31_ctrl->rolloff_update & 0x02) {
		vfe31_ctrl->rolloff_update &= (~0x02);
		vfe31_ctrl->outpath.out1.frame_drop_cnt++;
		return;
	}
	/* OPPO 2012-01-03 tanrf Add end */
...
static void vfe31_process_output_path_irq_2(uint32_t ping_pong)
{
	...
	*/
	...
	/* OPPO 2012-01-03 tanrf Add begin */
	if (vfe31_ctrl->rolloff_update & 0x04) {
		vfe31_ctrl->rolloff_update &= (~0x04);
		vfe31_ctrl->outpath.out2.frame_drop_cnt++;
		return;
	}
	/* OPPO 2012-01-03 tanrf Add end */
