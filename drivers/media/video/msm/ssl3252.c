...
#define CDBG_FLASH(fmt, args...) printk(KERN_INFO "ssl3252: " fmt, ##args) 
#else
...
int oppo_led_control(unsigned state)
{
	...
	case MSM_CAMERA_LED_RELEASE:
		...
		ret = gpio_direction_output(125, 0);//(125, 0)
		gpio_free(125);
		break;
...
exit:
	...
	ssl3252_shutdown();
	ssl3252_poweroff();
	// Added by yxq 2012-07-04 start
	ret = gpio_request(125, "flash_en");
	if(ret < 0)
		return -ENODEV;
	gpio_direction_output(125, 1);
	// Added by yxq 2012-07-04 end
	return ret;
}
