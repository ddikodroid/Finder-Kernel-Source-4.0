...
#include <linux/ion.h>
/*Added by Liu Jinshui 20120531 start*/
#ifdef CONFIG_OPPO_MODIFY
#include <linux/external_common_interface.h>
#endif
/*Added by Liu Jinshui 20120531 end*/
	...
	spinlock_t *pmem_spinlock)
{
	...
	hlist_for_each_entry_safe(region, node, n, ptype, list) {
		...
		if (region->info.type == pmem_type && (region->info.active == 1)) { /*OPPO ESD*/
			*regptr = *region;
			rc += 1;
			if (rc >= maxcount)
				break;
...
/*OPPO ESD*/
static int msm_pmem_frame_vpe_lookup(struct msm_sync *sync,
		unsigned long p0addr,
		unsigned long p1addr,
		unsigned long p2addr,
		int to_vpe)
{
	...
	hlist_for_each_entry_safe(region, node, n, &sync->pmem_frames, list) {
		if (p0addr == (region->paddr + region->info.planar0_off) &&
			p1addr == (region->paddr + region->info.planar1_off) &&
			p2addr == (region->paddr + region->info.planar2_off) &&
				(region->info.active == active)) {
			/* offset since we could pass vaddr inside
			 * a registerd pmem buffer
			 */
			if (to_vpe)
				region->info.active = 2;/*Active in vpe*/
			else
				region->info.active = 1;/*Active in vfe*/
			spin_unlock_irqrestore(&sync->pmem_frame_spinlock,
				flags);
			return 0;
		}
		...
		struct msm_frame *frame)
{
	...
	if (!qcmd->command) {
		pr_err("%s: cannot get frame, maybe unknown error occured! "
			"error_code %x \n", 
			__func__,
			qcmd->error_code);
			...
			void __user *arg)
{
	...
	qcmd_resp = __msm_control(sync,
				  ...
				  //qcmd, msecs_to_jiffies(10000));/*OPPO 2011-9-26 sunjianbo*/
		...
		if (data->type == VFE_MSG_COMMON) {
			...
			if ((sync->pp_frame_avail == 1) &&
				...
				(data->type == VFE_MSG_OUTPUT_P)) {
					...
					if(!msm_divert_frame(sync, data, &se))
		...
		struct msm_frame *frame)
{
	...
	if (MSM_CAM_Q_PP_MSG != qcmd->type) {
		...
		frame->frame_id = pframe->frame_id;
...
{
	struct msm_queue_cmd *qcmd =
		kzalloc(sizeof(struct msm_queue_cmd), GFP_KERNEL);
...
/* OPPO 2011-11-30 tanrf Delete begin */
#if 0
	if (!sync->pp_mask) {
...
#endif
/* OPPO 2011-11-30 tanrf Delete end */
	if (sync->pp_mask & PP_PREV) {
		...
		goto done;
	} else if (!(sync->pp_mask & PP_PREV)) {
			CDBG("%s: pp_prev is done.\n", __func__);	
/* OPPO 2011-03-24 tanrf Add begin : pop pp_prev to avoid buffer leakage */
			spin_lock_irqsave(&pp_prev_spinlock, flags);
			sync->pp_mask &= ~PP_PREV;
			if (sync->pp_prev) {
				CDBG("%s: sync->pp_prev not empty! try to release...\n", __func__);
				msm_enqueue(&sync->frame_q, &sync->pp_prev->list_frame);
				sync->pp_prev = NULL;
			}
			spin_unlock_irqrestore(&pp_prev_spinlock, flags);
/* OPPO 2011-03-24 tanrf Add end */
	}
	...
	unsigned long arg)
{
    ...
    case MSM_CAM_IOCTL_GET_MODULE_TYPE: { 
		rc = pmsm->sync->sctrl.s_get_module_type(argp);
		break;
    }
...
static int msm_camera_reset(struct msm_sync *sync)
{
	int rc = -EINVAL;
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_UNBLOCK_POLL_FRAME:
		...
		break;
	/*OPPO sunjianbo 2011-12-28 ESD*/
	case MSM_CAM_IOCTL_SENSOE_RESET:
		rc = msm_camera_reset(pmsm->sync);
		break;
	/*end*/
	default:
		break;
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_CTRL_COMMAND:
		...
		#if 0  /*OPPO 2011-3-29 sunjianbo edit*/
		mutex_lock(&ctrl_cmd_lock);
		rc = msm_control(ctrl_pmsm, 1, argp);
		mutex_unlock(&ctrl_cmd_lock);
		#else
		if(mutex_lock_interruptible(&ctrl_pmsm->control_lock))
			return -ERESTARTSYS;
		rc = msm_control(ctrl_pmsm, 1, argp);
		mutex_unlock(&ctrl_pmsm->control_lock);
		#endif
		break;
...
/*Added by Liu Jinshui 20120531 start*/
#ifdef CONFIG_OPPO_MODIFY
        pr_err("%s: will call external_common_camera with 0\n", __func__);
        external_common_camera(0);
        pr_err("%s: end call external_common_camera with 0\n", __func__);
#endif
/*Added by Liu Jinshui 20120531 end*/
	}
	mutex_unlock(&sync->lock);
...
static int msm_release_control(struct inode *node, struct file *filep)
{
	...
	if (!rc) {
		msm_queue_drain(&ctrl_pmsm->ctrl_q, list_control);
		mutex_destroy(&ctrl_pmsm->control_lock); /*OPPO 2011-3-29 sunjianbo add*/
		kfree(ctrl_pmsm);
	}
	return rc;
}
		...
		gfp_t gfp)
{
	...
	case VFE_MSG_OUTPUT_V:
		...
		if (sync->vpefn.vpe_cfg_update) {
			CDBG("dis_en = %d\n", *sync->vpefn.dis);
			if (*(sync->vpefn.dis) &&
				!msm_pmem_frame_vpe_lookup(/*OPPO ESD*/
					sync,vdata->phy.p0_phy,
					vdata->phy.p1_phy,vdata->phy.p2_phy,1)) {
	...
	enum msm_queue qtype, void *syncdata, void *ts, gfp_t gfp)
{
	...
	case VPE_MSG_OUTPUT_V:
		...
		msm_pmem_frame_vpe_lookup(sync,vdata->phy.p0_phy,
			vdata->phy.p1_phy, vdata->phy.p2_phy, 0);/*OPPO ESD*/
			...
			int is_controlnode)
{
	...
	if (!sync->core_powered_on && !is_controlnode) {
		...
		sync->pp_mask = 0;/*OPPO 2011-12-19 sunjianbo*/
...
/*Added by Liu Jinshui 20120531 start*/
#ifdef CONFIG_OPPO_MODIFY
        pr_err("%s: will call external_common_camera get rc\n", __func__);
        rc = external_common_camera(1);
        if(rc < 0){
            pr_err("%s: hdmi is on!\n", __func__);
            kfree(ctrl_pmsm);
            return rc;
        ...
        //msleep(2000);
        pr_err("%s: end call external_common_camera get rc\n", __func__);
#endif
/*Added by Liu Jinshui 20120531 end*/
	...
	mutex_init(&ctrl_pmsm->control_lock); /*OPPO 2011-3-29 sunjianbo add*/
