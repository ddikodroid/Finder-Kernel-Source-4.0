...
pm8xxx_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	dev_info(dev, "Rtc set time : Seconds value to be written to RTC = %lu\n", secs);
...
/* OPPO 2012-07-20 wangjc Delete begin for dell log when resume. */
#if 0
	dev_info(dev, "Rtc read time : secs = %lu, h:m:s == %d:%d:%d, d/m/y = %d/%d/%d\n",
...
pm8xxx_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	dev_info(dev, "Alarm read for - h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
		alarm->time.tm_hour, alarm->time.tm_min,
				alarm->time.tm_sec, alarm->time.tm_mday,
				alarm->time.tm_mon, alarm->time.tm_year);
...
static int __devinit pm8xxx_rtc_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "lrscp changed !!\n");
#ifdef CONFIG_PRINTK
	printk(KERN_ERR "printk is configured = %d", CONFIG_PRINTK);
	dev_err(&pdev->dev, "dev_err works\n");
#endif
	...
	if (rtc_write_enable == true){
		...
		dev_info(&pdev->dev, "support pm8xxx_rtc_set_time interface\n");//Roshan
	} 
...
static void pm8xxx_rtc_shutdown(struct platform_device *pdev)
{
	...
	if (!rtc_alarm_powerup) {
		...
		spin_lock_irqsave(&rtc_dd->ctrl_reg_lock, irq_flags);
		dev_info(&pdev->dev, "Don't support powerup alarm, Disabling alarm interrupts\n");
