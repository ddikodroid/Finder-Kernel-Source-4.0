...
#if 0
int SetClockActive(struct msm_fb_data_type *mfd)
{	
	int ret = 0;		
	printk(KERN_ERR "++SetClockActive!\n");	
	mfd->fbi->var.pixclock = clk_round_rate(pixel_mdp_clk,
		mfd->fbi->var.pixclock);	
	ret = clk_set_rate(pixel_mdp_clk, mfd->fbi->var.pixclock);
	if (ret) {		
...
out:
	return ret;}
#else
int SetClockActive(__u32 rate)
{	
	int ret = 0;		
	printk(KERN_ERR "++SetClockActive!\n");	
	rate = clk_round_rate(pixel_mdp_clk,
		rate);	
	ret = clk_set_rate(pixel_mdp_clk, rate);
	if (ret) {		
		pr_err("%s: Can't set MDP LCDC pixel clock to rate %u\n",
			__func__, rate);
	goto out;	
