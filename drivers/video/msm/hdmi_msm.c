...
//added by zhangyue on 2012-01- 18 for solve a suspend problem(dead lock)
DEFINE_MUTEX(hdmi_irq_mutex);
#endif
...
#ifdef CONFIG_OPPO_MODIFY
//added by zhangyue on 2012-01-12 for sync irq
void sync_control_irq(boolean on)
{
	static boolean irq_on = false;
	if(on == irq_on){
		DEV_INFO("the irq is already in %s state.\n", irq_on ? "enable" :"disable");
		return;
	}
	
	irq_on = on;
	mutex_lock(&hdmi_irq_mutex);
	if(on){
		enable_irq(hdmi_msm_state->irq);
		mutex_unlock(&hdmi_irq_mutex);
	}else{
...
#ifdef CONFIG_OPPO_MODIFY
		//added by zhengzk on 2012-06-16. If power is still on here, power off and reboot HPD.
		if(hdmi_msm_state->panel_power_on){
			kobject_uevent(external_common_state->uevent_kobj, KOBJ_OFFLINE);
			switch_set_state(&external_common_state->sdev, 0);
			DEV_INFO("HDMI HPD:power is still on, power off and reboot HPD\n");
			mdelay(500);
			envp[0] = "MHL_STATE=ON";
			envp[1] = NULL;
...
static void msm_hdmi_init_ddc(void)
{
	 ...
	 * 27 micro-seconds */
	#ifdef CONFIG_OPPO_MODIFY
	//modifed by zhangyue on 2011-12-27 for solving hdmi DDC timing problem start
	HDMI_OUTP_ND(0x027C, (1 << 16) | (0x26 << 0));
	//modifed by zhangyue on 2011-12-27 for solving hdmi DDC timing problem end
	#else
...
again:
	...
	time_out_count = wait_for_completion_interruptible_timeout(
		&hdmi_msm_state->ddc_sw_done, 2*HZ/5);	
	if(HDMI_INP_ND(0x0214) & 0x5)
		time_out_count = 1;
	#else
	time_out_count = wait_for_completion_interruptible_timeout(
...
static void hdcp_deauthenticate(void)
{
	...
	//added by zhengzk on 2012-05-17 for delay to reset HDCP
	mdelay(5);
...
static void hdmi_msm_hdcp_enable(void)
{
	...
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2011-11-08 for adding the ability of disable hdcp in runtime
	...
	#endif
#ifdef CONFIG_OPPO_MODIFY
		//added by zhengzk on 2012-05-31 to show the HDMI LOGO when disable HDCP in engineer mode
		envp[0] = "HDCP_STATE=PASS";
		envp[1] = NULL;
		DEV_INFO("HDCP disable,skip and send HDCP_PASS\n");
		kobject_uevent_env(external_common_state->uevent_kobj,
			KOBJ_CHANGE, envp);
#endif
		switch_set_state(&external_common_state->sdev, 1);
...
error:
	...
	if (hdmi_msm_state->hpd_during_auth) {
		...
		#ifdef CONFIG_OPPO_MODIFY
		//added by zhengzk on 2012-05-21 for TV needs time to wake up
		if(!hdmi_msm_state->panel_power_on){
			mdelay(1000);
		}
		//for some time the hdcp lost first and then hpd lost after the hdcp reauth work.
		if (hdmi_msm_state->panel_power_on && hdmi_msm_state->hpd_stable 
			&& external_common_state->hpd_state){
			queue_work(hdmi_work_queue,
			    &hdmi_msm_state->hdcp_reauth_work);	
		}
...
static void hdmi_msm_hpd_off(void)
{
	...
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2012-01-15 for debugging the hdmi irq problems
	sync_control_irq(false);
	#else
	disable_irq(hdmi_msm_state->irq);
	#endif
...
static int hdmi_msm_power_off(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2012-01-14 for debug, for the problem of can't change resolution. 
	//as here we don't think there is need to trigger hpd detect as our code will 
	//do it in mhl module.
...
static int __devinit hdmi_msm_probe(struct platform_device *pdev)
{
	...
	//deleted by zhangyue on 2012-01-14 for reduce Redundant code and work
	...
	if (hdmi_msm_has_hdcp()) {
		...
		if (external_common_state->hdcp_ctrl){
	...
	#ifndef CONFIG_OPPO_MODIFY
	//deleted by zhangyue on 2012-01-15 for delete Redundant code
...
static int __init hdmi_msm_init(void)
{
	...
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2011-01-04 for improving the problem(lose power after plug in MHL module)
	...
	*/
	...
	#ifdef CONFIG_OPPO_MODIFY
	//added by zhangyue on 2011-10-24 for adding the ability of disable the hdcp
	#ifdef zy_hdcp_ctrl
...
#endif
#ifdef CONFIG_OPPO_MODIFY
	hrtimer_init(&external_common_state->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	external_common_state->timer.function = external_common_cam_hpd_true;
#endif
