...
#include <linux/external_common_interface.h>
#endif
...
#ifdef zy_hdcp_ctrl
static ssize_t hdmi_common_hdcp_ctrl(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	uint8_t hdcp_ctrl = atoi(buf);
	
	DEV_DBG("%s: the original value is '%d' and write to %d \n", __func__,
			external_common_state->hdcp_ctrl, hdcp_ctrl);
	if(hdcp_ctrl && ! external_common_state->hdcp_ctrl){
		external_common_state->hdcp_ctrl = TRUE;
	...
	}else if(0 == hdcp_ctrl && external_common_state->hdcp_ctrl){
		external_common_state->hdcp_ctrl = FALSE;
		...
		DEV_DBG("%s: '%d' (unchanged)\n", __func__,
			external_common_state->hdcp_ctrl);
	}
	return ret;
}
#endif //zy_hdcp_ctrl_dbg
#ifdef zy_usrspace_enalbe_mhl_irq
static ssize_t common_mhl_irq_read(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	bool mhl_irq_init_state = false;
	ssize_t ret;
	mhl_irq_init_state = read_mhl_irq_Init_state();
	if(mhl_irq_init_state){
		ret = snprintf(buf, PAGE_SIZE, "%s\n","Inited mhl irq enabled");
	}else{
		ret = snprintf(buf, PAGE_SIZE, "%s\n","Not Inited: mhl irq disabled");
	}
	return ret;
}
static ssize_t common_mhl_irq_set(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	uint8_t enable_mhl_irq = atoi(buf);
	
	DEV_DBG("%s: ready to enable the mhl irq! \n", __func__);
	if(enable_mhl_irq){
		write_mhl_irq_Init_state();
	}
	return ret;
}
#endif
...
#ifdef CONFIG_OPPO_MODIFY
#ifdef zy_auto_hpd
//add the implement code for func external_common_enable_hpd_feature 
//on 2011-11-01
int external_common_enable_hpd_feature(bool on)
{
	char *envp[2];

	mutex_lock(&external_cam_state_mutex);
	if(external_common_state->camrea_state){
		mutex_unlock(&external_cam_state_mutex);
		envp[0] = "CAM_STATE=ON";
		envp[1] = NULL;
		DEV_DBG("MHL plug in, but camera is on .\n");
	...
	envp[0] = "MHL_STATE=ON";
	envp[1] = NULL;
	printk("zhengzk kernel MHL plug in.\n");
	kobject_uevent_env(external_common_state->uevent_kobj,
				   KOBJ_CHANGE, envp);	
...
EXPORT_SYMBOL(external_common_enable_hpd_feature);
enum hrtimer_restart external_common_cam_hpd_true(struct hrtimer *timer)
{
	//modified  by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem start
	if(is_mhl_connected()){
		external_common_enable_hpd_feature(true);
	}
	//external_common_enable_hpd_feature(true);
	//modified  by zhangyue on 2012-01-12 for tackling the camera and mhl coexist problem end
	return HRTIMER_NORESTART;
}

EXPORT_SYMBOL(external_common_cam_hpd_true);
...
int external_common_camera(bool on)
{
	...
	mutex_lock(&external_cam_state_mutex);
	if(is_mhl_connected() && external_common_state->hpd_feature_on){
		mutex_unlock(&external_cam_state_mutex);
		//added by zhangyue on 2012-03-07 for camera and mhl coexist problems start
		envp[0] = "MHL_IN_USE";
		envp[1] = NULL;
		DEV_INFO("Ready to open camera, but the MHL is in use,"
				   "If we want to use camera, we must disconncet the MHL .\n");
		kobject_uevent_env(external_common_state->uevent_kobj,
				   KOBJ_CHANGE, envp);
		//added by zhangyue on 2012-03-07 for camera and mhl coexist problems end
		return -EBUSY;
	}
	external_common_state->camrea_state = on;
	mutex_unlock(&external_cam_state_mutex);
	if(on){
		if (hrtimer_active(&external_common_state->timer))
			hrtimer_cancel(&external_common_state->timer);
	}else{
		hrtimer_start(&external_common_state->timer,
			ktime_set((3000 / 1000),
			(3000 % 1000) * 1000000), HRTIMER_MODE_REL);
	}
	return 0;
}
EXPORT_SYMBOL(external_common_camera);
//added by zhangyue on 2012-01-10 for camera and mhl coexist problems end
#endif
#endif
...
#endif
	//added by zhangyue on 2011-12-01 for making the write mode correct start
	msleep(200); //sleep 50 ms
	//addec by zhangyue on 2011-12-01 for making the write mode correct end
...
#ifdef zy_hdcp_ctrl
static DEVICE_ATTR(hdcp_ctrl, S_IRUSR |S_IWUSR, NULL, hdmi_common_hdcp_ctrl);
#endif
#ifdef zy_usrspace_enalbe_mhl_irq
static DEVICE_ATTR(mhl_irq_Init, S_IRUSR|S_IWUSR, common_mhl_irq_read, common_mhl_irq_set);
#endif
#endif
#ifdef CONFIG_FB_MSM_HDMI_3D
...
#ifdef zy_hdcp_ctrl
	&dev_attr_hdcp_ctrl.attr,
#endif
#ifdef zy_usrspace_enalbe_mhl_irq
	&dev_attr_mhl_irq_Init.attr,
#endif
#endif
#ifdef CONFIG_FB_MSM_HDMI_3D
...
static int hdmi_common_read_edid_block(int block, uint8 *edid_buf)
{
...
#ifdef CONFIG_OPPO_MODIFY
	//added by zhengzk on 2012-06-04 for repeat reading edid if checksum failed
	uint32 repeat_count;
#endif
#ifdef DEBUG
	const u8 *b = edid_buf;
	...
	for(repeat_count = 0; repeat_count < 3; repeat_count++)
	{
		...
		if (check_sum & 0xFF) {
			mdelay(250);
			DEV_ERR("EDID check sum error,count=%d\n", repeat_count);
...
int hdmi_common_read_edid(void)
{
	...
	if (status || !check_edid_header(edid_buf)) {
		...
		//deleted by zhengzk on 2012-05-30 for improve fault-tolerant ability
		goto error;
#endif
	}
...
bool hdmi_common_get_video_format_from_drv_data(struct msm_fb_data_type *mfd)
{
	...
	if (var->reserved[3]) {
		...
		case 1920:
			#ifdef CONFIG_OPPO_MODIFY
			//modified by zhangyue on 2011-02-08 for improving the problem(lose power after plug in MHL module)
