...
/* OPPO 2011-03-03 huanggd Add begin for oppo logo display */
#define FACTORY_IMAGE_FILE "/factorylogo.rle"
/* OPPO 2011-03-03 huanggd Add end */

/* OPPO 2011-10-22 zhangzr Add begin for oppo logo display */
#define MSM_BOOT_MODE__FACTORY	4
#define MSM_BOOT_MODE__WLAN		5
#define MSM_BOOT_MODE__RF		6
#define RF_IMAGE_FILE "/rflogo.rle"
#define WLAN_IMAGE_FILE "/wlanlogo.rle"
/* OPPO 2011-10-22 zhangzr Add end */
...
extern int get_boot_mode(void);//added by songxh for ftm display
...
static struct msm_fb_data_type *tp_mfd;//added by songxh  
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	int ret = -ENODEV;
	int bpp;
	...
	if (!load_565rle_image(INIT_IMAGE_FILE)) {	/* Flip buffer */	
	       ...
	       //added by zhangy on 2011-10-25 for the bug: MHL can not output video and the logo problem
	       if(mfd->panel.type != DTV_PANEL){//add by zhangyue for debugging
		#endif
			if (msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, mfd->op_enable)) {
				printk(KERN_ERR "msm_fb_register: can't turn on display!\n");
			}
		#ifdef CONFIG_OPPO_MODIFY
		//added by zhangy on 2011-10-25 for the bug: MHL can not output video and the logo problem			
		}			
...
//added by songxh begain for ftm display
/* OPPO 2012-02-07 zhangq Add begin for reason */
	tp_mfd = mfd;
/* OPPO 2012-02-07 zhangq Add end */
	switch (get_boot_mode())
	{
		case MSM_BOOT_MODE__FACTORY:
			load_565rle_image(FACTORY_IMAGE_FILE);
			printk(KERN_INFO"%s:  display factory mode logo\n\n", __func__);
			mdp_set_dma_pan_info(mfd->fbi, NULL, TRUE);
			schedule_delayed_work(&startup_refresh_work,  HZ/20);
			if (msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, mfd->op_enable)) 
			{
		...
		case MSM_BOOT_MODE__WLAN:
			load_565rle_image(WLAN_IMAGE_FILE);
			printk(KERN_INFO"%s:  display wlan mode logo\n\n", __func__);
			mdp_set_dma_pan_info(mfd->fbi, NULL, TRUE);
			schedule_delayed_work(&startup_refresh_work,  HZ/20);
			if (msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, mfd->op_enable)) 
			{
		...
		case MSM_BOOT_MODE__RF:
			load_565rle_image(RF_IMAGE_FILE);
			printk(KERN_INFO"%s:  display rf mode logo\n\n", __func__);
			mdp_set_dma_pan_info(mfd->fbi, NULL, TRUE);
			schedule_delayed_work(&startup_refresh_work,  HZ/20);
			if (msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi, mfd->op_enable)) 
			{
			...
			break;
		default:
			printk(KERN_INFO"%s:  normal boot\n", __func__);
			break;
	}	
//added by songxh end for ftm display	
#endif
	//modified by zhengzk for improving hpd feature on 2012-02-22 start
	if(mfd->panel.type == DTV_PANEL){
		var->reserved[3]= 34; //HDMI_VFRMT_1920x1080p30_16_9+1		
	}
	//modified by zhengzk for improving hpd feature on 2012-02-22 end
	ret = 0;
...
static int msm_fb_set_par(struct fb_info *info)
{
	...
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2012-01-17 for improving change resolution problem start
		(mfd->hw_refresh && (var->reserved[2]||(mfd->fb_imgType != old_imgType) ||
	#else
		(mfd->hw_refresh && ((mfd->fb_imgType != old_imgType) ||
	//modified by zhangyue on 2012-01-17 for improving change resolution problem end
	#endif
				(mfd->var_pixclock != var->pixclock) ||
...
#ifdef CONFIG_OPPO_MODIFY
int quamqamm_pclk[] = {25600000,6400000, 12800000, 17000000, 19200000, 21300000/*,42667000,43192000,48000000,53990000,64000000,76800000,16000000*/};

extern int SetClockActive(__u32 rate);
extern void StaticElectricityTest(void);
extern  void setup_acl_save(int on_off);
extern int acl_setup_flag;
#endif
			...
			unsigned long arg)
{
	...
	int ret = 0;
#ifdef CONFIG_OPPO_MODIFY
	static int pclk_type = 0;
//	int test = OPPO_SETUP_ACL_SAVE;
//	printk(KERN_INFO"%s:  OPPO_SETUP_ACL_SAVE = %d\n", __func__, test);
#endif
	switch (cmd) {
#ifdef CONFIG_FB_MSM_OVERLAY
	...
	case MSMFB_MDP_PP:
		...
		break;
#ifdef CONFIG_OPPO_MODIFY	
			case OPPO_CHANGE_LCD_PCLK:		 
#if 0
				pclk_type = *((int *)argp); 
		
				printk(KERN_ERR"pclk_type = %d	cmd = 0x%x", pclk_type, cmd);
				if(mfd == 0)
					printk(KERN_ERR"mfd is null\n");	
				mfd->panel_info.clk_rate = quamqamm_pclk[0];//[pclk_type];		
				mfd->fbi->var.pixclock = mfd->panel_info.clk_rate;		
				SetClockActive(mfd);		
#else
		{ 
			__u32  rate;
			...
			SetClockActive(rate);		
				...
				break;
			case OPPO_GET_LCD_PCLK:
				ret = copy_to_user(argp, &(mfd->panel_info.clk_rate), sizeof(mfd->panel_info.clk_rate));
				if(ret){
					printk(KERN_ERR
						"%s:OPPO_GET_LCD_PCLK ioctl failed \n", __func__);
						return ret;
				}
				break;
			case OPPO_STATIC_ELECTRICITY:
				StaticElectricityTest();
				break;
			case OPPO_SETUP_ACL_SAVE:
			{
				int setup_acl = *((int *)argp);
				if((setup_acl == 1) || (setup_acl == 0))
					setup_acl_save(setup_acl);
				break;
			}	
	case OPPO_GET_ACL_STATE:
		ret = copy_to_user(argp, &acl_setup_flag, sizeof(acl_setup_flag));
		if(ret){
			printk(KERN_ERR
				"%s:OPPO_GET_ACL_STATE ioctl failed \n", __func__);
				return ret;
		}
		break;
#endif
