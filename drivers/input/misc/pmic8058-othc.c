...
struct pm8058_othc {
       bool not_err_mode;  //liuyan 2012.6.15,for wrong headset mode
       bool had_inset;           //liuyan  2012.6.15,the symbol of the headset will keep when cut off the mhl power after check the vchg in interruput
	bool othc_sw_state;
	bool switch_reject;
...
extern int adc_get_vchg_mvolts(void);
...
//yanghai remove
#ifndef CONFIG_OPPO_MODIFY// yanghai removed
...
//		micbias_status = !!rc;//qualcom original
		micbias_status = !rc;//modified by liuyan in 2012-3-13
	} else {
		rc = gpio_get_value_cansleep(dd->ir_gpio);
		if (rc < 0) {
	...
	pr_err("yanghai get micbias stat=%d,  switch=%d\n",micbias_status,switch_status);
...
fail_ir_accessory:
 ...
 #ifndef CONFIG_OPPO_MODIFY// liuyan removed
...
static void detect_work_f(struct work_struct *work)
{
	...
	struct pm8058_othc *dd =
		container_of(work, struct pm8058_othc, detect_work.work);
	//liuyan,2012.6.15, for wrong headset mode
       vchg = adc_get_vchg_mvolts();
	if(vchg>3000){
	   if(dd->not_err_mode==true){
	   	pr_info("do not in headset mode after");
          //no headset connect ,so disable mic bias
		ret = pm8058_micbias_enable(OTHC_MICBIAS_1,
		OTHC_SIGNAL_OFF);
	if (ret)
		pr_err("%s: disabling  micbias 1 failed\n", __func__);
	   }
	if(dd->had_inset){
		pr_info("after cut off mhl power and recover the power");
              dd->othc_pdata->hd_switch_enable(0);
	...
	dd->not_err_mode=false;
	//liuyan,2012.6.15, for wrong headset mode end
...
//		goto fail_ir;
	...
	dd->othc_ir_state = !rc;
	if (dd->othc_ir_state) {
	// headset connect ,so enable mic bias
		ret = pm8058_micbias_enable(OTHC_MICBIAS_1,
		OTHC_SIGNAL_ALWAYS_ON);
	if (ret)
		pr_err("%s: Enabling micbias 1 failed\n", __func__);
	} else {
	//no headset connect ,so disable mic bias
		ret = pm8058_micbias_enable(OTHC_MICBIAS_1,
		OTHC_SIGNAL_OFF);
	...
	if (dd->othc_ir_state) {
		/* inserted */
		dd->othc_pdata->hd_switch_enable(1);
		rc = pm8058_accessory_report(dd, 1);
		if (rc)
			pr_err("Accessory could not be detected\n");
		dd->had_inset=true;
 ...
 */
//this place had modify to the original
...
static irqreturn_t pm8058_nc_ir(int irq, void *dev_id)
{
	...
	/* disable irq, this gets enabled in the workqueue */
	disable_irq_nosync(dd->othc_irq_ir);
     //liuyan 2012.615,if in headset mode
     vchg = adc_get_vchg_mvolts();
      if(vchg>3000){
          pr_info("do not in headset mode");
	   goto err_heaset_mode;
	  }
	  dd->not_err_mode=true;
     //liuyan 2012.615,if in headset mode
    //this place had modify to de quancommm original
	/* Check the MIC_BIAS status, to check if inserted or removed */
	rc = pm8xxx_read_irq_stat(dd->dev->parent, dd->othc_irq_ir);
	if (rc < 0) {
		pr_err("Unable to read IR status\n");
	...
	//dd->othc_ir_state = rc;
      dd->othc_ir_state = !rc;
#ifndef CONFIG_OPPO_MODIFY  //liuyan modify in 2012-4-12
	if (dd->othc_ir_state) {
		/* disable irq, this gets enabled in the workqueue */
		disable_irq_nosync(dd->othc_irq_ir);
...
othc_configure_hsed(struct pm8058_othc *dd, struct platform_device *pd)
{
	...
	dd->switch_reject = true;//yanghai add 
	...
	//dd->othc_irq_ir = platform_get_irq(pd, 1);//yanghai removed
	dd->othc_irq_ir = gpio_to_irq(191);//yanghai modify pm8058gpio19 to headset detect pin
	...
	ipd->dev.parent = &pd->dev;
       dd->had_inset =false;
	dd->not_err_mode= false;  //liuyan 2012.6.15,for wrong headset mode
	dd->othc_ipd = ipd;
	...
	if (dd->ir_gpio < 0) {
		pr_err("yanghai set othc_ir_gpio<0 \n");//yanghai add
		...
		pr_err("yanghai set othc_ir_gpio=%d \n",dd->ir_gpio);//yanghai add
    ...
    //this place had modify to qualcomm original
	...
	if (!rc) {
		pr_debug("Accessory inserted during boot up\n");
		pr_err("yanghai Accessory inserted during boot up\n");
		/* process the data and report the inserted accessory */
//		rc = pm8058_accessory_report(dd, 1);
//		if (rc)
//			pr_err("Unabele to detect accessory at boot up\n");
		
	//		if(dd->curr_accessory==OTHC_HEADSET)
	//			dd->switch_reject = false;//yanghai add 
		
		schedule_delayed_work(&dd->detect_work,
				msecs_to_jiffies(dd->detection_delay_ms));
	/* Disable the switch reject flag */
	dd->switch_reject = false;
	}else{
	//no headset connect ,so disable mic bias
	ret = pm8xxx_readb(dd->dev->parent, dd->othc_base + 1, &reg);
	if (ret< 0) {
		pr_err("PM8058 read failed\n");
	//	return rc;
	}
	...
	reg &= PM8058_OTHC_EN_SIG_MASK;
	reg |= (OTHC_SIGNAL_OFF << PM8058_OTHC_EN_SIG_SHIFT);

	ret = pm8xxx_writeb(dd->dev->parent, dd->othc_base + 1, reg);
	if (ret< 0) {
		pr_err("PM8058 write failed\n");
	//	return rc;
	}
	}
// liuyan modify in 2012-4-12 end
...
int get_head_set_status(void)
{
	int rc;
	struct pm8058_othc *dd = config[1];
	...
	rc = pm8xxx_read_irq_stat(dd->dev->parent, dd->othc_irq_ir);
	if (rc < 0) {
		pr_err("%s Unable to read IR status\n", __func__);
		return rc;
	}
...
EXPORT_SYMBOL(get_head_set_status);
