		  ...
		  unsigned int remain)
{
	void __iomem *base = host->base;
	char *ptr = buffer;
/* OPPO 2012-6-12 Modify by yansen for improve efficiency, make use of full empty fifo. 
...
#endif
/* OPPO 2012-6-12 Modify by yansen end */
	mb();
...
#ifdef CONFIG_OPPO_MODIFY //add by liuhd for bcm4330 wifi suspend		
	if (host ->pdev_id == 4 && !host->clks_on){
	//	host->clks_on = 1 ;
		msmsdcc_sdio_al_lpm(mmc, false);
	}
...
#ifdef CONFIG_OPPO_MODIFY //add by liuhd for bcm4330 wifi suspend
	 //Comtech add for skip mmc_rescan when suepend
        if (host->pdev_id == 4) {
            pr_err("Comtech test mmc%d pm, skip mmc_rescan when suspend \n", mmc->index);
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		rc = mmc_suspend_host(mmc);
#endif
#endif //end add by liuhd for bcm4330 wifi suspend
		}
