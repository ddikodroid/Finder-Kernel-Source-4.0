...
//oppo.Roshan modify for timer unstable condition
static cycle_t msm_dgt_read(struct clocksource *cs)
{
	...
	if (clock_state->stopped){
		now = clock_state->stopped_tick >> clock->shift;
	} else {
		now = (msm_read_timer_count(clock, GLOBAL_TIMER) +
			clock_state->sleep_offset) >> clock->shift;
	}

	if ((cs->cycle_last & cs->mask) > now){
		delta = (cs->cycle_last & cs->mask) - now;
		if(delta < 10 * 6750000){
			//printk(KERN_INFO"%s missed %lld cycles, last %lld, now %lld", 
			//			cs->name, delta, cs->cycle_last, now);
			//dump_stack();
			now = cs->cycle_last & cs->mask;
		} else {
			//printk(KERN_INFO"%s overflows every 10 minutes", cs->name);
		}
	}
	return now;
}
