...
#define CONFIG_OPPO_MODIFY
#endif

#include <linux/memblock.h>

#ifdef CONFIG_OPPO_MODIFY //huqiao
#ifdef CONFIG_TOUCHSCREEN_CANDO_TS
#include <linux/input/cando_ts.h>
#endif

#ifdef CONFIG_TOUCHSCREEN_ATMEL_1386
#include <linux/input/atmel_ts_1386.h>
#endif

#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_TM1400_I2C_RMI
#include <linux/synaptics_i2c_rmi.h>
#endif

#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_TM1734_I2C_RMI
#include <linux/synaptics_i2c_rmi.h>
#endif
#if defined(CONFIG_TOUCHSCREEN_ATMEL_QT602240) || defined(CONFIG_TOUCHSCREEN_ATMEL_QT602240E)
...
#include <linux/ion.h>
#include <mach/ion.h>
/* Added by yxq 2012-04-14 start */
#ifdef CONFIG_OPPO_MODIFY
#include <linux/i2c/ssl3252.h>
#endif
/* Added by yxq 2012-04-14 end */

extern int sus_flag;//huqiao

#define MSM_SHARED_RAM_PHYS 0x40000000
#define MDM2AP_SYNC 129

#define GPIO_ETHERNET_RESET_N_DRAGON	 30
#define LCDC_SPI_GPIO_CLK			36		//	73		//changed by huangtongfeng
#define LCDC_SPI_GPIO_CS			39		//72		//changed by huangtongfeng
#define LCDC_SPI_GPIO_MOSI			35		//	70		//changed by huangtongfeng
...
int factory_mode = 0;
//liuyan 2012.6.22
bool change_to_uart = false;
bool open_uart_switch =false;
//
...
#ifdef CONFIG_OPPO_MODIFY//huqiao		
static struct platform_device OPPO_Gpio_keys;
#endif
...
/*OPPO 2012-04-19 songxh add for ftm mode begain*/
static struct kobject *systeminfo_kobj;
//static char serial_number[64] = {0}; /* add by songxh for save serial number. */

unsigned int app_bootmode = 0; //moved by songxh 
static char ftm_serial_number[64] = {0}; 
static ssize_t ftm_sn_show(struct kobject *kobj, struct kobj_attribute *attr,
			char *buf){
    return snprintf(buf, 4096, "%s\n", ftm_serial_number);
}

static ssize_t ftm_sn_store(struct kobject *kobj, struct kobj_attribute *attr,
			 const char *buf, size_t count){
	if(count >= sizeof(ftm_serial_number))
		count = sizeof(ftm_serial_number) - 1;
	
	strncpy(ftm_serial_number, buf, count - 1);
	ftm_serial_number[count-1] = '\0';
	return 0;
}

struct kobj_attribute ftm_sn_attr = {
    .attr = {"ftm_serialno", 0644},
	
    .show = &ftm_sn_show,
    .store = &ftm_sn_store
};
/*OPPO 2011-12-12 Jiangsm add end*/

#define NORMAL_MODE  			0
#define FTM_RECOVERY_MODE 	2
#define FTM_CHARGE_MODE 		3
#define FTM_AT_MODE 			4
#define FTM_WLAN_MODE 		5
#define FTM_RF_MODE 			6
...
{
	if (!strcmp(s, "normal"))
		ftm_mode = NORMAL_MODE;
	else if (!strcmp(s, "factory2"))
		ftm_mode = FTM_AT_MODE;
	else if (!strcmp(s, "recovery"))
		ftm_mode = FTM_RECOVERY_MODE;
	else if (!strcmp(s, "charge"))
		ftm_mode = FTM_CHARGE_MODE;
	else if (!strcmp(s, "ftmwifi"))
		ftm_mode = FTM_WLAN_MODE;
	else if (!strcmp(s, "ftmrf"))
		ftm_mode = FTM_RF_MODE;
	else 
		ftm_mode = NORMAL_MODE;
...
#define pcb_serial_number  "212001-SC"
static ssize_t pcb_sn_show(struct kobject *kobj, struct kobj_attribute *attr,
							char *buf)
{
	return sprintf(buf, "%s\n", pcb_serial_number);
}

struct kobj_attribute pcb_sn_attr = {
    .attr = {"pcb_sn", 0644},
...
//	&startupmode_attr.attr,
//	&appboot_attr.attr,
//	&reset_reason_attr.attr,
//	&sn_attr.attr,
	&ftm_sn_attr.attr,
	&ftmmode_attr.attr,
	&pcb_sn_attr.attr,
...
/*OPPO 2012-04-19 songxh add for ftm mode end*/
...
#endif
#if defined(CONFIG_OPPO_MODIFY)  //huqiao
//add for get hw version
static int  get_hw_version(void)
{
	char * ptr;
	ptr = strstr(saved_command_line,"androidboot.hardversion=");
	if(ptr == NULL)
	{
	    
	    printk("null hw_version");
		return 2;
	}
	
	ptr += strlen("androidboot.hardversion=");
	//printk("huqiao ---------%s---",ptr);
	if(!strncmp(ptr , "two", 3))
	{
		return 2;
	}
	else if (!strncmp(ptr , "five", 4))
	{
		return 2;
	}
	else if (!strncmp(ptr , "three", 5))
	{
		return 2;
	}else if (!strncmp(ptr , "six", 3))
	{
		return 2;
	}else if (!strncmp(ptr , "seven", 5))
	{
		return 2;
	}else if (!strncmp(ptr , "eight", 5))
	{
		return 2;
	}else if (!strncmp(ptr , "nine", 4))
	{
		return 2;
	}else if (!strncmp(ptr , "ten", 3))
	{
		return 2;
	}
	else if (!strncmp(ptr , "four", 4))
	{
		return 2;
	}
	else
...
#endif
static int __init IsUartEnable(void)
{	    
    char * ptr;	    
    ptr = strstr(boot_command_line,"androidboot.uart=");
    if(ptr == NULL)	    
    {	
        return 0;    
    }	
     
    printk("zhang yuan fei ---------%s---\n",ptr);
    
    ptr += strlen("androidboot.uart=");
...
#ifdef CONFIG_OPPO_MODIFY
static int __init isFactoryMode(void)
{	    
    char * ptr;	    
    ptr = strstr(boot_command_line,"androidboot.factorymode=");
    if(ptr == NULL)	    
    {	
        return 0;    
    } 
     
    ptr += strlen("androidboot.factorymode=");    
...
#ifdef CONFIG_OPPO_MODIFY
//add USB switch channel control function
enum usb_switch_type {
       /*USB switch default gpio97 0,gpio100 0 */
	USB_SWITCH_DEFAULT,
	/*USB switch uart gpio97 0,gpio100 1 */
	USB_SWITCH_USB,
	/*USB switch headset gpio97 1,gpio100 0 */
	USB_SWITCH_HEADSET,
	/*USB switch uart gpio97 1, gpio100 1 */
	USB_SWITCH_UART,
	/*USB switch MHL device*/
	USB_SWITCH_MHL,
};

#define USB_SWITCH_GPIO97 97
#define USB_SWITCH_GPIO100 100

#define USB_SWITCH_GPIO97_CONFIG  GPIO_CFG(USB_SWITCH_GPIO97, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
#define USB_SWITCH_GPIO100_CONFIG  GPIO_CFG(USB_SWITCH_GPIO100, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
static int usb_switch_change(enum usb_switch_type switch_type)
{
	int ret=0;
	
	pr_info("%s: usb switch type =%d\n",__func__,switch_type);
	ret = gpio_tlmm_config(USB_SWITCH_GPIO97_CONFIG, GPIO_CFG_ENABLE);
	if (ret){
		printk(KERN_ERR "%s:GPIO97 gpio_tlmm_config failed!(%#x)=%d\n",
			__func__, USB_SWITCH_GPIO97_CONFIG, ret);
		return -1;
	}

	ret = gpio_tlmm_config(USB_SWITCH_GPIO100_CONFIG, GPIO_CFG_ENABLE);
	if (ret){
		printk(KERN_ERR "%s:GPIO100 gpio_tlmm_config failed!(%#x)=%d\n",
			__func__, USB_SWITCH_GPIO100_CONFIG, ret);
		return -1;
	}
	
	switch(switch_type){
	case USB_SWITCH_USB:
		gpio_set_value(USB_SWITCH_GPIO97, 0);
		gpio_set_value(USB_SWITCH_GPIO100, 1);
		change_to_uart = false;
		break;
	case USB_SWITCH_HEADSET:
		gpio_set_value(USB_SWITCH_GPIO97, 1);
		gpio_set_value(USB_SWITCH_GPIO100, 0);
		change_to_uart = false;
		break;
	case USB_SWITCH_UART:
		/*make sure HEADSET mode  first to avoid run to USB status */
		gpio_set_value(USB_SWITCH_GPIO97, 1);
		gpio_set_value(USB_SWITCH_GPIO100, 0);
...
unsigned reboot_reason = 0;

#define IMEM_BASE           			0x2A05F000
#define RESTART_REASON_ADDR 	0x65C
static unsigned int get_app_bootmode(void)
{
	...
	reboot_reason = readl(restart_reason_addr);
	//writel(0x00, restart_reason_addr);	
	iounmap(restart_reason_addr);

	 printk("%s reboot_reason = %u\n", __func__, reboot_reason);
...
/* OPPO 2012-05-22 Saturn Add begin for add /proc/last_kmsg intf */
...
/* OPPO 2012-05-22 Saturn Add end */

/*add following for BRCM4330 start by liuhd*/
#ifdef CONFIG_BCMDHD
static int msm_sdcc_setup_gpio(int dev_id, unsigned int enable);
#define WL_REG_ON  66 //WL_REG_ON , the GPIO number of qualcomm platform, modified +++
#define WL_SLOT_NUM 4
#ifdef CONFIG_BCMDHD_OOB
#define WLAN_OOB_IRQ 57 //wl oob irq number
#endif
...
int brcm_wifi_power(int on)
{
	...
	if (on) {
	  msm_sdcc_setup_gpio(WL_SLOT_NUM, 1); // the slot num of sdio, maybe different for customer
	   ...
	   //msleep(150);
   	  ...
   	  gpio_set_value(WL_REG_ON,0);
   	  msleep(100);
	  msm_sdcc_setup_gpio(WL_SLOT_NUM, 0);//off the GPIO
	  //msleep(10);
	}
	msleep(150);
...
#endif
/*add following for BRCM4330 end by liuhd*/
//add by liuhd for wifi power
#ifndef CONFIG_OPPO_MODIFY
#define GPIO_WL_RSTN_GPIO (66)
#define WL_HOST_WAKE_PD_GPIO    (57)	  

#define WL_REG_ON_PU	GPIO_CFG(GPIO_WL_RSTN_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
#define WL_REG_ON_PD	GPIO_CFG(GPIO_WL_RSTN_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)
#define WL_HOST_WAKE_PD		GPIO_CFG(WL_HOST_WAKE_PD_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)

static void __init wifi_oob_gpio_init(void)
{
	int ret;
		
		ret = gpio_tlmm_config(WL_HOST_WAKE_PD, GPIO_CFG_ENABLE);
		if (ret) {
			printk(KERN_ERR "%s:gpio_tlmm_config(%#x)=%d\n",
					__func__, WL_HOST_WAKE_PD, ret);
	}
}
...
void bcm_wlan_power_on(int on)
	{
		...
		/* 1--power on, 2--reset on */
		
		printk(KERN_INFO"%s-----on=%d\n",__func__,on);
		
		if (on == 1) {
			ret = gpio_tlmm_config(WL_REG_ON_PU, GPIO_CFG_ENABLE);
			if (ret) {
				printk(KERN_ERR "%s:gpio_tlmm_config(%#x)=%d\n",
					__func__, WL_REG_ON_PU, ret);
			}
			gpio_set_value(GPIO_WL_RSTN_GPIO, 0);
			msleep(100);
			gpio_set_value(GPIO_WL_RSTN_GPIO, 1);
			msleep(200);
		}
		else if (on == 2) {
			ret = gpio_tlmm_config(WL_REG_ON_PU, GPIO_CFG_ENABLE);
			...
			printk(KERN_ERR "ERROR:NO POWER ON OR RESET ON !!!\n");
		}
...
void bcm_wlan_power_off(int off)
	{
		...
		/* 1--power off, 2--reset off */
		
		printk(KERN_INFO"%s-----off=%d\n",__func__,off);
		if (off == 1) {
			ret = gpio_tlmm_config(WL_REG_ON_PD, GPIO_CFG_ENABLE);
			if (ret) {
				printk(KERN_ERR "%s:gpio_tlmm_config(%#x)=%d\n",
					__func__, WL_REG_ON_PD, ret);
			}
			gpio_set_value(GPIO_WL_RSTN_GPIO, 0);
			msleep(50);
		}
			...
			printk(KERN_ERR "ERROR:NO POWER OFF OR RESET OFF !!!\n");
		}
	}
...
#ifdef CONFIG_OPPO_MODIFY
//add by huanghao 2011.11.02 OTG no function, set vdd_cx sleep vol to 0.5v
//static int usb_phy_susp_dig_vol = 750000;
static int usb_phy_susp_dig_vol = 500000;
//end by huanghao 2011.11.02
#endif
...
static int msm_hsusb_ldo_enable(int on)
{
	...
	if (!uart_flag)
			usb_switch_change(USB_SWITCH_USB);
...
#endif
	.chg_connected		 = smb346_chg_connected,
};
#endif
...
/* Deleted by yxq 2012-04-14 start */
#ifndef CONFIG_OPPO_MODIFY
...
/* Modified by yxq 2012-04-14 start */
#ifndef CONFIG_OPPO_MODIFY
...
#endif
/* Modified by yxq 2012-04-14 end */
...
/* Added by yxq 2012-04-14 start */
#ifdef CONFIG_OPPO_MODIFY
static int imx111_config_camera_on_gpios(void)
{
	int rc = 0;
...
static void imx111_config_camera_off_gpios(void)
{
	config_gpio_table(MSM_CAM_OFF);
}
#endif
...
#else
#define SUBCAM_PWD 45
#define SUBCAM_RST 72

void s5k8aay_reset(void)
{
	gpio_direction_output(SUBCAM_PWD, 1);
	msleep(10);
	gpio_direction_output(SUBCAM_RST,0);
	msleep(10);
	gpio_direction_output(SUBCAM_RST,1);
}

static int config_camera_on_gpios_s5k8aay(void)
{
	int rc = 0;
	rc = config_gpio_table(MSM_CAM_ON);
	if (rc < 0) {
	...
	gpio_direction_output(SUBCAM_PWD, 0);
	gpio_direction_output(SUBCAM_RST, 0);
	msleep(20);
	return rc;
}

static void config_camera_off_gpios_s5k8aay(void)
{
	gpio_set_value_cansleep(SUBCAM_PWD, 1);
	config_gpio_table(MSM_CAM_OFF);
	return;
}
#endif
...
/* Added by yxq 2012-04-14 start */
#ifdef CONFIG_OPPO_MODIFY
struct msm_camera_device_platform_data imx111_camera_device_data = {
	.camera_gpio_on  = imx111_config_camera_on_gpios,
	.camera_gpio_off = imx111_config_camera_off_gpios,
	.ioext.csiphy = 0x04800000,
	.ioext.csisz  = 0x00000400,
	.ioext.csiirq = CSI_0_IRQ,
	.ioclk.mclk_clk_rate = 24000000,
	.ioclk.vfe_clk_rate  = 228570000,  //192000000->228570000
#ifdef CONFIG_MSM_BUS_SCALING
	.cam_bus_scale_table = &cam_bus_client_pdata,
#endif
};
...
static struct msm_camera_sensor_platform_info imx111_sensor_board_info = {
	.mount_angle = 90
};
...
static struct msm_camera_sensor_info msm_camera_sensor_imx111_data = {
	...
	.vcm_pwd		= 1,
	.vcm_enable		= 0,
	.pdata			= &imx111_camera_device_data,
	.resource		= msm_camera_resources,
	.num_resources		= ARRAY_SIZE(msm_camera_resources),
	.flash_data		= &flash_imx111,
	.sensor_platform_info = &imx111_sensor_board_info,
	.csi_if			= 1
};

struct platform_device msm_camera_sensor_imx111 = {
...
struct msm_camera_device_platform_data msm_camera_device_data_s5k8aay = {
	.camera_gpio_on  = config_camera_on_gpios_s5k8aay,
	.camera_gpio_off = config_camera_off_gpios_s5k8aay,
	.is_subcamera = 1,
	.camera_hw_reset = s5k8aay_reset,
	.ioext.csiphy = 0x04900000,
	.ioext.csisz  = 0x00000400,
	.ioext.csiirq = CSI_1_IRQ,
	.ioclk.mclk_clk_rate = 24000000,
...
static struct msm_camera_sensor_platform_info s5k8aay_sensor_board_info = {
	.mount_angle = 270
};
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k8aay_data = {
	.sensor_name	= "s5k8aay",
	.sensor_reset	= SUBCAM_RST,
	.sensor_pwd	= SUBCAM_PWD,
	.vcm_pwd	= 1,
	.vcm_enable	= 0,
	.pdata		= &msm_camera_device_data_s5k8aay,
	.resource	= msm_camera_resources,
	.num_resources	= ARRAY_SIZE(msm_camera_resources),
	.flash_data	= &flash_s5k8aay,
	.sensor_platform_info = &s5k8aay_sensor_board_info,
	.csi_if		= 1
};

struct platform_device msm_camera_sensor_s5k8aay = {
...
static struct i2c_board_info msm_subcamera_boardinfo[] __initdata = {
	#ifdef CONFIG_OPPO_MODIFY
	{
...
/* Modified by yxq 2012-04-14 start */
	...
	.clk_freq = 100000,
#else
	.clk_freq = 300000,//Modified by yxq from 400k to 300k, 2012-07-16
#endif
/* Modified by yxq 2012-04-14 start */
	.src_clk_rate = 24000000,
	.msm_i2c_config_gpio = gsbi_qup_i2c_gpio_config,
...
#ifdef CONFIG_OPPO_MODIFY
//add by chendx for config GSBI11 i2c
static struct msm_i2c_platform_data msm_gsbi11_qup_i2c_pdata = {
	.clk_freq = 100000,//100khz//400000,			
	.src_clk_rate = 24000000,
	//.clk = "gsbi_qup_clk",
	//.pclk = "gsbi_pclk",
	.msm_i2c_config_gpio = gsbi_qup_i2c_gpio_config,
...
#ifdef CONFIG_OPPO_MODIFY
//modified by huangtongfeng start

static int pmic_backlight_gpio = GPIO_BACKLIGHT_PWM0;
...
static struct mipi_dsi_panel_platform_data novatek_pdata = {
	...
	.gpio = &pmic_backlight_gpio, 		//wangcy add for pwm
};
...
#define SYNAPTICS_1734_REG_MAP_START		0xDD
#define ATMEL_FAMILY_ID		0x00
...
static struct regulator *vreg_2P8V = NULL;
static struct regulator *vreg_1P8V = NULL;
static struct regulator *vreg_0P8V = NULL;

int Check_Touchscreen_type(int type)
	{
		...
		case TS_ATMEL_224E:
			msgs[0].addr = 0x4a;
			msgs[1].addr = 0x4a;
			out_buf[0] = ATMEL_FAMILY_ID;
	
			if (i2c_transfer(adapter, msgs, 2) == 2) {
				printk("Atmel 224E detected\n");
				ret = 1;
			}
			break;
		...
		case TS_SYNA_1734:
			msgs[0].addr = 0x20;
			msgs[1].addr = 0x20;
			out_buf[0] = SYNAPTICS_1734_REG_MAP_START;
	
			if (i2c_transfer(adapter, msgs, 2) == 2) {
				printk("Syna 1734 detected\n");
				ret = 1;
			}
			break;
		default:
...
EXPORT_SYMBOL(Check_Touchscreen_type);
static int OPPO_TS_power(int on)
{
	int ret = 0;
	

      //Syna_hw_version = get_hw_version();
    hw_version = get_hw_version();
	if(vreg_2P8V == NULL)
	{		
		vreg_2P8V = regulator_get(NULL, "8901_l3");						
		if (IS_ERR(vreg_2P8V)) {
			return PTR_ERR(vreg_2P8V);
		}

		ret = regulator_set_voltage(vreg_2P8V,
				2850000,
				2850000);
		if (ret) {
			pr_err("%s: unable to set the voltage for regulator"
				"vreg_2P8V\n", __func__);
			regulator_put(vreg_2P8V);
			return ret;
		}
	}
	...
	if(vreg_1P8V == NULL)
	{		
			vreg_1P8V = regulator_get(NULL, "8058_l20");
		if (IS_ERR(vreg_1P8V)) {
			return PTR_ERR(vreg_1P8V);
		}
	

		ret = regulator_set_voltage(vreg_1P8V,
				1800000,
				1800000);
		if (ret) {
			pr_err("%s: unable to set the voltage for regulator"
				"vreg_1P8V\n", __func__);
			regulator_put(vreg_1P8V);
			return ret;
		}
	}
	if(vreg_0P8V == NULL)
	{
		...
		if(hw_version >= 1)
		{
			vreg_0P8V = regulator_get(NULL, "8058_l19");
		}
		...
		if (IS_ERR(vreg_0P8V)) {
			return PTR_ERR(vreg_0P8V);
		}

		ret = regulator_set_voltage(vreg_0P8V,
				3000000,
				3600000);
		if (ret) {
			pr_err("%s: unable to set the voltage for regulator"
				"vreg_0P8V\n", __func__);
			regulator_put(vreg_0P8V);
			return ret;
		}
	}
	if(on == 0xEF) {  //reset
		ret = regulator_disable(vreg_2P8V);
		if (ret) {
			pr_err("%s: Unable to disable the regulator:"
				"vreg_2P8V\n", __func__);
			return ret;
		}
		//regulator_put(vreg_Syna_2P8V);

		msleep(60);

		ret = regulator_enable(vreg_2P8V);
		if (ret) {
			pr_err("%s: unable to enable regulator"
				"vreg_2P8V\n", __func__);
			regulator_put(vreg_2P8V);
	...
	if (on) {
		...
		ret = regulator_enable(vreg_1P8V);
		if (ret) {
			pr_err("%s: unable to enable regulator"
				"vreg_1P8V\n", __func__);
			regulator_put(vreg_1P8V);
		...
		ret = regulator_enable(vreg_0P8V);
		...
		ret = regulator_disable(vreg_0P8V);
		...
		ret = regulator_disable(vreg_1P8V);
		if (ret) {
			pr_err("%s: Unable to disable the regulator:"
				"vreg_1P8V\n", __func__);
			return ret;
		}
		//regulator_put(vreg_Syna_1P8V);
		mdelay(10);
		ret = regulator_disable(vreg_2P8V);
		if (ret) {
			pr_err("%s: Unable to disable the regulator:"
...
#define OPPO_TS_ATTN		GPIO_CFG(TS_PEN_IRQ_GPIO, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_10MA)


static int OPPO_TS_Gpio_Setup(void)
{
	int rc;

	/* configure touchscreen interrupt gpio */
	rc =  gpio_tlmm_config(OPPO_TS_ATTN, GPIO_CFG_ENABLE);
	if (rc) {
...
#if defined (CONFIG_TOUCHSCREEN_SYNAPTICS_TM1734_I2C_RMI)
static struct synaptics_i2c_rmi_platform_data Synaptics_1734_data[] = {
	{
		.version = 0x0101,
		.power = OPPO_TS_power,
		.flags = SYNAPTICS_SNAP_TO_INACTIVE_EDGE,
		.inactive_left = -0 * 0x10000 / 1052,
		.inactive_right = -0 * 0x10000 / 1052,
		.inactive_top = -0 * 0x10000 / 1744,
		// .inactive_bottom = -64 * 0x10000 / 1744,
		.inactive_bottom = -0 * 0x10000 / 1744,
	},
	{
...
static struct i2c_board_info Synaptics_1734_board_info[] = {
	{
		I2C_BOARD_INFO(SYNAPTICS_I2C_RMI_NAME, 0x20),
		.platform_data = &Synaptics_1734_data,
		.irq = MSM_GPIO_TO_INT(TS_PEN_IRQ_GPIO),
	}
};
#endif
#if defined (CONFIG_TOUCHSCREEN_ATMEL_QT602240E)



static struct qt602240_platform_data ATMEL_QT602240E_data[] = {
	{
	.x_line = 19,
	.y_line = 11,
	.x_size = 1052,
...
static struct i2c_board_info ATMEL_QT602240E_board_info[] = {
	{
		I2C_BOARD_INFO(ATMEL_QT602240E_NAME, 0x4a),
		.platform_data = &ATMEL_QT602240E_data,
		.irq = MSM_GPIO_TO_INT(TS_PEN_IRQ_GPIO),
	}
};
#endif

#if defined (CONFIG_TOUCHSCREEN_ATMEL_QT602240)
static struct qt602240_platform_data ATMEL_QT602240_data[] = {
	{
	.x_line = 19,
	.y_line = 11,
	.x_size = 1052,
...
static struct i2c_board_info ATMEL_QT602240_board_info[] = {
	{
		I2C_BOARD_INFO(ATMEL_QT602240E_NAME, 0x4a),
		.platform_data = &ATMEL_QT602240_data,
		.irq = MSM_GPIO_TO_INT(TS_PEN_IRQ_GPIO),
	}
};
#endif
...
#if CONFIG_OPPO_MODIFY
//huyu------------add
static struct regulator *vreg_9[2], *vreg_11[2];//*vreg_10[2],
static int ipowon = 0;

int sensor_power_on(void)
{
	int rc = 0;
	//static int ipowon = 0;
	char *msm_VREG_L9A = "8058_l9";
	//char *msm_VREG_L10A = "8058_l10";
	char *msm_VREG_L11A = "8058_l11";

	
	printk( "%s: ipowon  = %d\n", __func__,ipowon);

	//if(ipowon == 1) return rc;
	//ipowon = 1;
	//printk( "%s: ipowon  = %d\n", __func__,ipowon);
#if 0
	if(hw_version < 2)
	{
		vreg_10[ipowon] = regulator_get(NULL, msm_VREG_L10A);
		if (IS_ERR(vreg_10[ipowon])) {
			rc = PTR_ERR(vreg_10[ipowon]);
		...
		if (!rc)
		{
			rc = regulator_set_voltage(vreg_10[ipowon], 2850000, 2850000);
		}
		else {
			printk( "%s: regulator_set_voltage %s = %d\n", __func__,
				msm_VREG_L10A, rc);
			regulator_put(vreg_10[ipowon]);
			return rc;
		}
		
		if (!rc)
		{
			rc = regulator_enable(vreg_10[ipowon]);
		}
		else {
...
#endif
	vreg_9[ipowon] = regulator_get(NULL, msm_VREG_L9A);
	if (IS_ERR(vreg_9[ipowon])) {
		rc = PTR_ERR(vreg_9[ipowon]);
		printk( "%s: regulator_get %s = %d\n", __func__,
			msm_VREG_L9A, rc);
	}
	...
	if (!rc)
	{
		rc = regulator_set_voltage(vreg_9[ipowon], 2800000, 2800000);
	}
	else {
		printk( "%s: regulator_set_voltage %s = %d\n", __func__,
			msm_VREG_L9A, rc);
	...
	if (!rc)
	{
		rc = regulator_enable(vreg_9[ipowon]);
	}
	else {
		printk( "%s: regulator_enable %s = %d\n", __func__,
			msm_VREG_L9A, rc);
	...
	vreg_11[ipowon] = regulator_get(NULL, msm_VREG_L11A);
	if (IS_ERR(vreg_11[ipowon])) {
		rc = PTR_ERR(vreg_11[ipowon]);
		printk( "%s: regulator_get %s = %d\n", __func__,
			msm_VREG_L11A, rc);
	}
	...
	if (!rc)
	{
		rc = regulator_set_voltage(vreg_11[ipowon], 1800000, 1800000);
	}
	else {
		printk( "%s: regulator_set_voltage %s = %d\n", __func__,
			msm_VREG_L11A, rc);
	...
	if (!rc)
	{
		rc = regulator_enable(vreg_11[ipowon]);
	}
	else {
		printk( "%s: regulator_enable %s = %d\n", __func__,
			msm_VREG_L11A, rc);
...
unget1:
	regulator_put(vreg_11[ipowon]);
unget2:
	regulator_put(vreg_9[ipowon]);
	return rc;

}
#endif

int sensor_power_off(void)
{
	int ret = 0;
	
	//printk( "%s: ipowon  = %d\n", __func__,ipowon);

	//if(ipowon == 1) return ret;
	ipowon --;
	//printk( "%s: ipowon  = %d\n", __func__,ipowon);
#if 0
	if(hw_version < 2)
	{
		if(vreg_10[ipowon]){
			ret = regulator_disable(vreg_10[ipowon]);
			if (ret) {
				pr_err("%s: Unable to disable the regulator:"
					"vreg_10\n", __func__);
				return ret;
...
#endif	
	if(vreg_9[ipowon]){
		ret = regulator_disable(vreg_9[ipowon]);
		if (ret) {
			pr_err("%s: Unable to disable the regulator:"
				"vreg_9\n", __func__);
			return ret;
	...
	if(vreg_11[ipowon]){
		ret = regulator_disable(vreg_11[ipowon]);
		if (ret) {
			pr_err("%s: Unable to disable the regulator:"
				"vreg_11\n", __func__);
			return ret;
...
#if 0
static int light_sensor_setup_power(struct device *dev)
{
	printk("nothing-to do \n");
	return 0 ;
}
static void light_sensor_shutdown_power(struct device *dev)
{
	printk("nothing-to do \n");
}
#endif
#if 1
static struct light_sensor_platform_data light_sensor_pdata = {
	.power_on = sensor_power_on,//light_sensor_setup_power,
	.power_off = sensor_power_off,
};
static struct i2c_board_info light_sensor_i2c_info[] __initdata = {
	{
		I2C_BOARD_INFO("tsl2773", 0x39),
		.platform_data = &light_sensor_pdata,
		.irq = PM8058_GPIO_PM_TO_SYS(33),
	},
};
#endif
...
static struct i2c_board_info __initdata inv_mpu_i2c4_boardinfo[] = {
	{
		...
		.irq = PM8058_GPIO_PM_TO_SYS(16),
		.platform_data = &mpu3050_data,
	},
	{
		I2C_BOARD_INFO("lis3dh", 0x19),
		//.irq = PM8058_GPIO_PM_TO_SYS(22),
...
static struct regulator *vreg_ldo19_adc;
#define VREG_LDO19_VOL_MIN	3050000 /* uV */
#define VREG_LDO19_VOL_MAX	3050000 /* uV */
#define VREG_LDO18_VOL_MIN	2200000 /* uV */
#define VREG_LDO18_VOL_MAX	2200000 /* uV */

static void smb346_charger_ldo_power(int on)
{
	int rc = 0;
	...
	if (on) {
		...
		rc = regulator_set_voltage(vreg_ldo18_adc, VREG_LDO18_VOL_MIN, VREG_LDO18_VOL_MAX);
		if (rc) {
			printk(KERN_ERR"%s  vreg_ldo18_adc regulator_set_voltage error!\n", __func__);
		}

        rc = regulator_enable(vreg_ldo18_adc);
		if (rc < 0) {
			printk(KERN_ERR"%s   vreg_ldo18_adc  regulator_enable error!\n", __func__);
		}
	}else {
		if(vreg_ldo18_adc) {
			rc = regulator_disable(vreg_ldo18_adc);
			if (rc) {
				printk(KERN_ERR"%s  vreg_ldo18_adc regulator_disable error!\n", __func__);
			}
...
static void smb346_charger_ldo_init(void)
{
	if(hw_version > 1) {
		int rc = 0;
		
		vreg_ldo19_adc = regulator_get(NULL, "8058_l19");
	
		rc = regulator_set_voltage(vreg_ldo19_adc,VREG_LDO19_VOL_MIN,VREG_LDO19_VOL_MAX);			
		if (rc) {
			printk(KERN_ERR"%s vreg_ldo19_adc   regulator_set_voltage error!\n", __func__);
		}

        rc = regulator_enable(vreg_ldo19_adc);
		if (rc < 0) {
			printk(KERN_ERR"%s  vreg_ldo19_adc regulator_enable error!\n", __func__);
		}

		rc = regulator_disable(vreg_ldo19_adc);
		if (rc) {
			printk(KERN_ERR"%s  vreg_ldo19_adc regulator_disable error!\n", __func__);
		}
	}
}

static void msm_hsusb_enable(int on)
{
	int ret=0;
	
	if(on)
	{
		ret = usb_switch_change(USB_SWITCH_USB);
		if(ret)
	  		pr_err("%s: usb switch change on failed!\n",__func__);
	}else{
		if (uart_flag) 
			ret =	usb_switch_change(USB_SWITCH_UART);	
		else
		 	ret =usb_switch_change(USB_SWITCH_DEFAULT);	 
		 	 
	  if(ret)
	  		pr_err("%s: usb switch change off failed!\n",__func__);
	}
}
/*uart switch feature*/
static void msm_hsuart_enable(int on)
{	
	int ret=0;
	
	if(on)	
	{	open_uart_switch = true;
	       if(change_to_uart){
		    ret = usb_switch_change(USB_SWITCH_UART);	  
		    if(ret)	  		
			    pr_err("%s: uart switch change on failed!\n",__func__);	
		    pr_info("%s: hsuart enalbe on\n",__func__);
	       }
	}else{	 
	      if(change_to_uart){
		        ret = usb_switch_change(USB_SWITCH_DEFAULT);	  
		       if(ret)	  		
			       pr_err("%s: uart switch change off failed!\n",__func__);	
	        	pr_info("%s: hsuart enalbe off\n",__func__);
			}
		open_uart_switch = false;
   }
}
/*uart switch feature*/


//add by chendx for smb346 charger 2012-04-17
static struct smb346_battery_platform_data smb346_battery_data = {
	.ldo_power = smb346_charger_ldo_power,
	.ldo_init = smb346_charger_ldo_init,
	.hsusb_enable = msm_hsusb_enable,
	.hsuart_enable = msm_hsuart_enable,
	.avail_chg_sources = AC_CHG |USB_CHG,
};

static struct platform_device smb346_battery_device = {
	.name = "smb346-battery",
	.id = -1,
	.dev = {
		.platform_data = &smb346_battery_data,
	}
};
#endif
#endif
#ifdef CONFIG_ACCESSORY_BATTERY
static struct platform_device accessory_battery_device = {
...
static struct rpm_regulator_init_data rpm_regulator_init_data[] = {
	...
	RPM_LDO(PM8058_L1,  0, 1, 0, 1200000, 1200000, LDO300HMIN),  // yxq
...
/* Modified by yxq 2012-04-14 end */
	...
	RPM_LDO(PM8058_L11, 0, 1, 0, 1800000, 1800000, LDO150HMIN),		//huyu
	#ifdef CONFIG_OPPO_MODIFY //modifed by zhangyue on 2011-11-04
	...
	RPM_LDO(PM8058_L15, 0, 1, 0, 3000000, 3000000, LDO300HMIN),//huangtongfeng modify 2012.4.13
	...
	RPM_LDO(PM8058_L19, 0, 1, 0, 3000000,3600000, LDO150HMIN),
	...
	RPM_LDO(PM8058_L25, 0, 1, 0, 1200000, 1200000, LDO150HMIN),  // yxq
...
//changed by bosj  for bt sleep begin
...
static struct resource bluesleep_resources[] = {	
	...
	{		.name	= "host_wake",	
		.start	= MSM_GPIO_TO_INT(58),	
		.end	= MSM_GPIO_TO_INT(58),		
		.flags	= IORESOURCE_IRQ,	
	},
};
...
static struct msm_adc_channels msm_adc_channels_data[] = {
	...
	{"ref_325mv", CHANNEL_ADC_325_REF, 0, &xoadc_fn, CHAN_PATH_TYPE14,
		ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE2, scale_default},
	//add by chendx 2010010 新增ADC 通道
	#ifdef	CONFIG_OPPO_MODIFY
	{"amux5", CHANNEL_ADC_AMUX5, 0, &xoadc_fn, CHAN_PATH_TYPE6, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	{"amux6", CHANNEL_ADC_AMUX6, 0, &xoadc_fn, CHAN_PATH_TYPE7, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	{"amux7", CHANNEL_ADC_AMUX7, 0, &xoadc_fn, CHAN_PATH_TYPE8, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	{"amux8", CHANNEL_ADC_AMUX8, 0, &xoadc_fn, CHAN_PATH_TYPE9, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	{"amux9", CHANNEL_ADC_AMUX9, 0, &xoadc_fn, CHAN_PATH_TYPE10, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	//{"unused", CHANNEL_ADC_UNUSED, 0, &xoadc_fn, CHAN_PATH_TYPE13, ADC_CONFIG_TYPE2, ADC_CALIB_CONFIG_TYPE3, scale_default}, 
	#endif
};
...
static void pmic8058_xoadc_mpp_config(void)
{
	...
	struct pm8xxx_mpp_init_info xoadc_mpps[] = {
		PM8058_MPP_INIT(XOADC_MPP_3, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH5,
							AOUT_CTRL_DISABLE),
		#ifndef	CONFIG_OPPO_MODIFY  //modify by chendx 2011-09-26 for read vchg voltage					
		PM8058_MPP_INIT(XOADC_MPP_5, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH9,
							AOUT_CTRL_DISABLE),
		#endif
		PM8058_MPP_INIT(XOADC_MPP_7, A_INPUT, PM8XXX_MPP_AIN_AMUX_CH6,
							AOUT_CTRL_DISABLE),
		#ifndef	CONFIG_OPPO_MODIFY  //modify by chendx 2011-09-26 make channel for checking vchg voltage
		//Todo
...
{
	#ifndef CONFIG_OPPO_MODIFY
	//del by chendx 2011-10-04  ldo18 on make sure 2.2v always exist
	int rc;
...
/* Added by yxq 2012-04-14 start */
#ifdef CONFIG_OPPO_MODIFY
	&msm_camera_sensor_s5k8aay,
	&msm_camera_sensor_imx111,
#endif
/* Added by yxq 2012-04-14 end */
#endif
#ifdef CONFIG_MSM_GEMINI
...
#if defined(CONFIG_OPPO_MODIFY) && defined(CONFIG_BATTERY_BQ27541_OPPO)//chendexiang ,20110218,add
...
#endif
#ifndef CONFIG_OPPO_MODIFY//yanghai remove
...
/*OPPO 2012-04-26 end by wjp */
		...
		{ /* yanghai use headset detect gpio*/
		...
		{ /*huyu, for lightsensor interrupt*/
			PM8058_GPIO_PM_TO_SYS(33),
			{
				.direction		= PM_GPIO_DIR_IN,
				.pull			= PM_GPIO_PULL_NO,
				.vin_sel		= 2,
		...
		{/*zhengzk disable GPIO36 on 2012-06-28*/
...
#ifdef CONFIG_OPPO_MODIFY//huqiao
static struct gpio_keys_button OPPO_Gpio_buttons[] = {
	{
		.code       = KEY_VOLUMEUP,
		.gpio       = PM8058_GPIO_PM_TO_SYS(19),
		.active_low = 0,
		.desc       = "volume up",
		.type       = EV_KEY,
		.wakeup     = 1,
...
static struct gpio_keys_platform_data OPPO_Gpio_keys_platform_data = {
	.buttons  = OPPO_Gpio_buttons,
	.nbuttons = ARRAY_SIZE(OPPO_Gpio_buttons),
};
...
static struct platform_device OPPO_Gpio_keys = {
	...
	.id   = -1,
	.dev  = {
		.platform_data = &OPPO_Gpio_keys_platform_data,
	},
};
#endif
...
static struct pm8xxx_vibrator_platform_data pm8058_vib_pdata = {
	.initial_vibrate_ms  = 0,//500,  modified by zhangyf for remove kernel first vibration sound
	.level_mV = 3000,
...
//qualcom riginal
#ifndef CONFIG_OPPO_MODIFY// liuyan modify 2012-3-13
...
static struct hsed_bias_config hsed_bias_config = {
	...
	.othc_headset = OTHC_HEADSET_NO,
	.othc_lowcurr_thresh_uA = 100,
	.othc_highcurr_thresh_uA = 550,
	.othc_hyst_prediv_us = 7800,
	.othc_period_clkdiv_us = 62500,
	.othc_hyst_clk_us = 15600,
	.othc_period_clk_us = 312500,
	.othc_wakeup = 1,
};
#endif
static struct othc_hsed_config hsed_config_1 = {
	 ...
	 */
	.detection_delay_ms = 150,   //liuyan 2012.6.6,for quick plug headset
	/* Switch info */
...
//added by zhangqiang on 2012-05-30 for avoid wrong switch
#define EXT_BQ27541_CHG_VALID_MPP 10
#define MPP11_VALID_GPIO PM8058_MPP_PM_TO_SYS(EXT_BQ27541_CHG_VALID_MPP)

//yanghai add for 12001 switch at 2012.03.13
static void hd_switch(int enable)
{
	//added by zhangqiang on 2012-05-30 for avoid wrong switch start
	//int value;
	...
	if(value < 0)
	{
		printk("Get value error value =%d\n", value);
		return ;
	}
	else if(value == 0)
	{
		printk("USB still insert, should not change switch mode.\n");
		return ;
	}*/
	//added by zhangqiang on 2012-05-30 for avoid wrong switch end

	switch(enable)
	{
	case 0:
		usb_switch_change(USB_SWITCH_DEFAULT);
		break;
	case 1:
		usb_switch_change(USB_SWITCH_HEADSET);
		break;
	default:
		usb_switch_change(USB_SWITCH_DEFAULT);
		break;	
...
#ifdef CONFIG_OPPO_MODIFY //Van modified 2012.05.07
...
#ifdef CONFIG_OPPO_MODIFY
#ifdef CONFIG_SMB346_CHARGER
static int smb346_charger_power_on(int on)
{
      return 0;
}

static bool smb346_get_ftm_mode(void)
{
     if (factory_mode == 1){
	 	return true;
     }else{
...
#define CHG_SMB346_IRQ 26 //pmic8058 gpio26
/*smb346 charging chip pdata 2012-02-18*/
static struct smb346_platform_pdata smb346_charger_pdata = {
	.smb346_irq_gpio		= PM8058_GPIO_PM_TO_SYS(CHG_SMB346_IRQ - 1),
	.power	                     = smb346_charger_power_on,
	.get_ftm_mode	       = smb346_get_ftm_mode, 
};

static struct platform_device smb346_charger_device = {
	.name = "smb346-charger",
	.id = -1,
	.dev = {
		.platform_data = &smb346_charger_pdata,
	}
};
#endif
#endif
...
#ifdef CONFIG_OPPO_MODIFY
//added by zhangyue for add mhl driver
#ifdef CONFIG_MHL_Sii8334
//#define CI2CA  true  // CI2CA depend on the CI2CA pin's level
#ifdef CI2CA 
#define SII8334_plus 0x04  //Define sii8334's I2c Address of all pages by the status of CI2CA. the original value is 0x02
#else
#define SII8334_plus 0x00  //Define sii8334's I2c Address of all pages by the status of CI2CA.
#endif
//#define PAD_DBG_MHL         1
#ifdef PAD_DBG_MHL
#define MHL8334_RST_GPIO 85
#define MHL8334_IRQ_GPIO 169
#else
#define MHL8334_RST_GPIO 40
#define MHL8334_IRQ_GPIO 124
#endif
...
#ifdef mhl_power_dbg
static struct  regulator * vreg_12 = NULL;
static struct  regulator * vreg_24 = NULL;
#endif
struct mhl8334_platform_data{
	void (* reset)(void);
#ifdef mhl_power_dbg
...
{
	int ret = 0;
	//added by zhangyue on 2011-11-21 for a warning
	static bool FirstBoot = false;
	//added by zhangyue on 2011-11-21 end
	char *msm_VREG_L12A = "8058_l12";
	char *msm_VREG_L24A = "8058_l24";
	
	if(NULL == vreg_12){
		vreg_12 = regulator_get(NULL, msm_VREG_L12A);
		if(IS_ERR(vreg_12)){
			ret = PTR_ERR(vreg_12);
			pr_err( "%s: regulator_get %s = %d\n", __func__,
				msm_VREG_L12A, ret);
	...
	if(NULL == vreg_24){
		vreg_24 = regulator_get(NULL, msm_VREG_L24A);
		if(IS_ERR(vreg_24)){
			ret = PTR_ERR(vreg_24);
			pr_err( "%s: regulator_get %s = %d\n", __func__,
				msm_VREG_L24A, ret);
	...
	ret = regulator_set_voltage(vreg_12, 1800000, 1800000);
	if(ret){
		pr_err( "%s: regulator_set_voltage %s = %d\n", __func__,
			msm_VREG_L12A, ret);
		goto ERR_SET_L12;
	...
	ret = regulator_set_voltage(vreg_24, 1200000, 1200000); 
	if(ret){
		pr_err( "%s: regulator_set_voltage %s = %d\n", __func__,
			msm_VREG_L24A, ret);
		goto ERR_SET_L24;
	...
	if(!FirstBoot){
		...
		gpio_free(MHL8334_RST_GPIO);
	}
	return 0;
	
ERR_SET_L24:
	regulator_disable(vreg_12);
ERR_SET_L12:
	regulator_put(vreg_24);
	vreg_24 = NULL;
ERR_GET_L24:
	regulator_put(vreg_12);
	vreg_12 = NULL;
	return ret;
}


static int mhl_power_off(void)
{
	int ret = 0;
		
	ret = regulator_disable(vreg_24);
	if(ret){
		pr_err("%s: Unable to disable the regulator:"
			"vreg_24\n", __func__);
	}
	
	ret = regulator_disable(vreg_12);
	if(ret){
		pr_err("%s: Unable to disable the regulator:"
			"vreg_12\n", __func__);
	}
	
	gpio_request(MHL8334_RST_GPIO, "mhl_sii8334_sys_rst_n");
	
	return ret;
}
static int mhl_irq_config(void)
{
	int rc;
	gpio_request(MHL8334_IRQ_GPIO, "mhl_sii8334_irq");
	rc = gpio_direction_input(MHL8334_IRQ_GPIO);
	if(rc){
		printk(KERN_ERR "set MHL8334_IRQ_GPIO input failed.\n ");
		goto unrequest;
	}
	return 0;
unrequest:
	gpio_free(MHL8334_IRQ_GPIO);
	return rc;
}
#endif
static void Sii8334_reset(void)
{
	...
	printk(KERN_INFO "In func Sii8334_reset()!\n");

	gpio_request(MHL8334_RST_GPIO, "mhl_sii8334_sys_rst_n");
	rc = gpio_direction_output(MHL8334_RST_GPIO, 0);
	if(rc){
		goto unrequest;
	}
	msleep(100);
	gpio_set_value(MHL8334_RST_GPIO, 1);	
	gpio_free(MHL8334_RST_GPIO);
	return;
unrequest:
	printk(KERN_ERR "Set GPIO MHL8334_RST_GPIO output failed!\n");
	gpio_free(MHL8334_RST_GPIO);
}
...
static struct mhl8334_platform_data Sii8334_data = {
	.reset = Sii8334_reset,
#ifdef mhl_power_dbg
	.mhl_power_on = mhl_power_on,
	.mhl_power_off = mhl_power_off,
	.mhl_irq_config = mhl_irq_config,
#endif
	//added by zhangyue on 2012-05-03 for making USB switch to mhl function
	.mhl_swtich = mhl_swtich,
};

static struct i2c_board_info mhl_8334_board_info[] = {
	{
	 .type = "sii8334_PAGE_TPI",
	 .addr = 0x39 + SII8334_plus, 
	 .irq = MSM_GPIO_TO_INT(MHL8334_IRQ_GPIO),  //define the interrupt signal input pin
...
static struct i2c_registry msm8x60_i2c_devices[] __initdata = {
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_1386
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI3_QUP_I2C_BUS_ID,
		atmel_1386_board_info,
		ARRAY_SIZE(atmel_1386_board_info),
	},
#endif
#ifdef CONFIG_MHL_Sii8334
	{
		...
		mhl_8334_board_info,
		ARRAY_SIZE(mhl_8334_board_info),
	},
#endif
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_TM1734_I2C_RMI
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI3_QUP_I2C_BUS_ID,
		Synaptics_1734_board_info,
		ARRAY_SIZE(Synaptics_1734_board_info),
	},
#endif
#ifdef CONFIG_TOUCHSCREEN_ATMEL_QT602240E
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI3_QUP_I2C_BUS_ID,
		ATMEL_QT602240E_board_info,
		ARRAY_SIZE(ATMEL_QT602240E_board_info),
	},
#endif
#ifdef CONFIG_TOUCHSCREEN_ATMEL_QT602240
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI3_QUP_I2C_BUS_ID,
		ATMEL_QT602240_board_info,
		ARRAY_SIZE(ATMEL_QT602240_board_info),
	},
#endif
...
//huyu------------add
...
#endif
	{
		I2C_SURF | I2C_FFA,
		MSM_GSBI5_QUP_I2C_BUS_ID,
		light_sensor_i2c_info,
		ARRAY_SIZE(light_sensor_i2c_info),
	},
#endif
//huyu------------add
...
#endif
//added by zhangyue on 2011-11-21 for debugging the mhl suspend problem
#ifdef CONFIG_MHL_Sii8334
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI12_QUP_I2C_BUS_ID,
...
#ifdef CONFIG_OPPO_MODIFY
{
	I2C_SURF | I2C_FFA | I2C_FLUID,
	MSM_GSBI8_QUP_I2C_BUS_ID,
	msm_subcamera_boardinfo,
	ARRAY_SIZE(msm_subcamera_boardinfo),
},
#endif
/* Added by yxq 2012-04-14 end */
};
...
static void __init msm8x60_init_buses(void)
{
	...
	msm_gsbi5_qup_i2c_device.dev.platform_data = &msm_gsbi5_qup_i2c_pdata;	//huyu
	#ifdef CONFIG_OPPO_MODIFY
	//add by chendx for config GSBI11 i2c
...
static struct mmc_platform_data msm8x60_sdc4_data = {
	...
	.translate_vdd  = msm_sdcc_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
#ifdef CONFIG_BCMDHD /*For Broadcom wifi SDIO detect purpose*/
	.status 				= brcm_wifi_status, //add liuhd
	.register_status_notify = brcm_wifi_status_register, //add liuhd
#endif
	.msmsdcc_fmin	= 400000,
	.msmsdcc_fmid	= 24000000,
	.msmsdcc_fmax	= 48000000,
...
#ifdef CONFIG_BCMDHD
		gpio_tlmm_config(GPIO_CFG(WL_REG_ON, 0, GPIO_CFG_OUTPUT,
						GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE); //initialize the WL_REG pin, Tom+++
		gpio_tlmm_config(GPIO_CFG(WLAN_OOB_IRQ, 0, GPIO_CFG_INPUT,
						GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE); //initialize the WLAN_OOB_IRQ pin, Tom+++
		gpio_set_value(WL_REG_ON, 0); //set 0 Tom+++
#endif
#endif
#ifdef CONFIG_MMC_MSM_SDC5_SUPPORT
	/*
...
{
	#ifdef CONFIG_OPPO_MODIFY
	//modified by zhangyue on 2011-10-15
	//for turn off the hdmi 5v because we don't use the 5v any more
	#else
...
/****the following codes is changed by bosj***begin**/

#define GPIO_BT_SYSRSTN_OPPO (69)
#define GPIO_BT_REG_OPPO     (67)
#define MSM_WAKE_BT 		 (94)
#define BT_WAKE_MSM     	 (58)

	int rc_69;
    int rc_67;
	rc_69 = gpio_tlmm_config(GPIO_CFG(GPIO_BT_SYSRSTN_OPPO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_12MA), 0);
	rc_67 = gpio_tlmm_config(GPIO_CFG(GPIO_BT_REG_OPPO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_12MA), 0);

	printk("bosj***the state of bluetooth is %d\n", on);
	if (rc_69)
		pr_err("%s:0111 for BT_RSTN_OPPO %d request"
		"failed\n",__func__,GPIO_BT_SYSRSTN_OPPO);
	if (rc_67)
		pr_err("%s:0111 for BT_RSTN_OPPO %d request"
		"failed\n",__func__,GPIO_BT_REG_OPPO);

	if (on)
	{

		gpio_set_value_cansleep(GPIO_BT_SYSRSTN_OPPO,1);
		gpio_set_value_cansleep(GPIO_BT_REG_OPPO, 1);
		//gpio_set_value_cansleep(BT_WAKE_MSM,1);

	}
	else
	{
		gpio_set_value_cansleep(GPIO_BT_SYSRSTN_OPPO,0);
		gpio_set_value_cansleep(GPIO_BT_REG_OPPO, 0);
		//gpio_set_value_cansleep(BT_WAKE_MSM,0);
...
#ifdef CONFIG_OPPO_MODIFY
    factory_mode = isFactoryMode();//added by songxh for factory mode 
#endif 
  
    pmic_reset_irq = PM8058_IRQ_BASE + PM8058_RESOUT_IRQ;
    uart_flag = IsUartEnable();  
...
#ifdef CONFIG_OPPO_MODIFY	
	/*
	modify by tanglc on 2012-02-13 for idle current on Airplane mode(1.1mA increase), for we 
	donn't use any devices attached to EBI2, we should not call the 'msm8x60_init_ebi2' func
	*/
	if (machine_is_msm8x60_surf() || machine_is_msm8x60_rumi3() || 
		machine_is_msm8x60_sim() || machine_is_msm8x60_fluid() || machine_is_msm8x60_dragon())
#else
	if (machine_is_msm8x60_surf() || machine_is_msm8x60_ffa() ||
...
#ifdef CONFIG_SMB346_CHARGER
//add by chendx for smb346 charger 2012-04-17
	  platform_device_register(&smb346_battery_device);
#endif
...
#ifdef CONFIG_SMB346_CHARGER
//add by chendx for smb346 charger 2012-04-17
	  platform_device_register(&smb346_charger_device);
#endif

#ifdef CONFIG_ACCESSORY_BATTERY
	//add by chendx 2012-04-26
	platform_device_register(&accessory_battery_device);
#endif
...
/* OPPO 2012-04-25 songxh add begin for systeminfo */
...
/* OPPO 2012-04-25 songxh add end */

    ram_console_debug_init();
}

// mwalker add interface start reason -->
#ifndef MODULE
...
/*OPPO 2012-04-26 add by wjp */
int get_sim_detect_gpio_value(void)
{
	int value;
	value = gpio_get_value_cansleep(PM8058_GPIO_PM_TO_SYS(26));
	return value;
}
/*OPPO 2012-04-26 end by wjp */
