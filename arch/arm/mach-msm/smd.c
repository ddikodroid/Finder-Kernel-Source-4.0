...
/*OPPO add by wjp 2012-07-24 begin*/
#include <linux/wakelock.h>
/*OPPO add by wjp 2012-07-24 end*/
...
/* OPPO 2012-07-18 wangjc Add begin for check whether name is NULL. */
	if(!name) {
		pr_err("%s name is NULL.\n", __func__);
		return -ENODEV;
	}
/* OPPO 2012-07-18 wangjc Add end */
	...
	if (!smsm_info.state) {
		...
		if (apps & SMSM_RESET) {
			/* If we get an interrupt and the apps SMSM_RESET
			   ...
			   app's reset ack. */
			 /*OPPO del by wjp 2012-07-24 begin*/
			//if (!cpu_is_msm8960() && !cpu_is_msm8930())
				//apps &= ~SMSM_RESET;
			/*OPPO del by wjp 2012-07-24 end*/
