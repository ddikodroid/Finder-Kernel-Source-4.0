...
extern int q6_shutdown(void);
extern int q6_powerup(void);

static ssize_t modem_proc_write( struct file *filp, const char __user *buff,
                        unsigned long len, void *data )
{
	...
	{
		q6_shutdown();
		modem_shutdown(&subsys_8660_modem);
	}
	else if(proc_data[0] == 'S')
	{
		modem_powerup(&subsys_8660_modem);
		q6_powerup();
	}
	else if(proc_data[0] == 'R')
	{
		subsystem_restart("modem");
	}
...
static int init_modem_proc(void)
{
	int ret=0;
	
	proc_entry = create_proc_entry( "modem_shutdown", 0666, NULL );
	
	if (proc_entry == NULL) {
		ret = -ENOMEM;
		pr_err("%s creat proc entry failed\n", __func__);
	} else {
		proc_entry->write_proc = modem_proc_write;
