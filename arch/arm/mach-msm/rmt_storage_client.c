		...
		struct msm_rpc_xdr *xdr)
{
    ...
    //TODO: register another cb handler, and don't use open cb. suchangyu.
	if (!strncmp(path, "/oppo_cmd/backup_rf", MAX_PATH_NAME) ||
        !strncmp(path, "/oppo_cmd/backup_all", MAX_PATH_NAME) ||
        !strncmp(path, "/oppo_cmd/restore", MAX_PATH_NAME) ||
        !strncmp(path, "/oppo_cmd/checksum", MAX_PATH_NAME)){
        //NOTICE: ²»ÉêÇëclient, Ö±½ÓÖ¸¶¨ÊÂ¼þ. suchangyu.
        pr_info("%s: special handler for oppo cmd. path %s\n", __func__, path);
        event_args->id = RMT_STORAGE_OPEN;
        memcpy(event_args->path, path, len);
        cid = 1; // a dummy clinet id.
