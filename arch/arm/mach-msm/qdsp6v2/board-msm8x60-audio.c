...
#define GPIO_CLASS_D0_EN_OPPO (29) //define GPIO_29 to CLASS_D1 ENABLE
...
static int config_class_d0_gpio(int enable)
{
	...
	if (enable) {
		rc = gpio_request(GPIO_CLASS_D0_EN_OPPO, "CLASSD1_EN");
		if (rc) {
			pr_err("%s: spkr pamp gpio %d request"
			"failed\n", __func__, GPIO_CLASS_D0_EN_OPPO);
			return 0;
		}
		gpio_direction_output(GPIO_CLASS_D0_EN_OPPO, 1);
		gpio_set_value_cansleep(GPIO_CLASS_D0_EN_OPPO, 1);
	} else {
		gpio_set_value_cansleep(GPIO_CLASS_D0_EN_OPPO, 0);
		gpio_free(GPIO_CLASS_D0_EN_OPPO);
	}
	return 0;
#endif
}
...
//yanghai add
//loop main mic test
static struct adie_codec_action_unit loopmic_48KHz_osr256_actions[] =
	LOOPMIC_PRI_MONO_OSR_256;

static struct adie_codec_hwsetting_entry loopmic_settings[] = {
	{
		.freq_plan = 48000,
		.osr = 256,
		.actions =loopmic_48KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(loopmic_48KHz_osr256_actions),
	}
};

static struct adie_codec_dev_profile loopmic_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = loopmic_settings,
	.setting_sz = ARRAY_SIZE(loopmic_settings),
};

static struct snddev_icodec_data snddev_loopmic_data = {
	.capability = (SNDDEV_CAP_TX | SNDDEV_CAP_VOICE),
	.name = "main_loopback_tx",
	.copp_id = 1,
	.profile = &loopmic_profile,
	.channel_mode = 1,
	.default_sample_rate = 48000,
	.pamp_on = msm_snddev_enable_amic_power,
	.pamp_off = msm_snddev_disable_amic_power,
...
static struct platform_device msm_loopmic_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_loopmic_data },
};
//loop main mic test
//loop sencond mic test start
static struct adie_codec_action_unit loopsmic_48KHz_osr256_actions[] =
	LOOPSMIC_PRI_MONO_OSR_256;

static struct adie_codec_hwsetting_entry loopsmic_settings[] = {
	{
		.freq_plan = 48000,
		.osr = 256,
		.actions =loopsmic_48KHz_osr256_actions,
		.action_sz = ARRAY_SIZE(loopsmic_48KHz_osr256_actions),
	}
};

static struct adie_codec_dev_profile loopsmic_profile = {
	.path_type = ADIE_CODEC_TX,
	.settings = loopsmic_settings,
	.setting_sz = ARRAY_SIZE(loopsmic_settings),
};

static struct snddev_icodec_data snddev_loopsmic_data = {
	.capability = (SNDDEV_CAP_TX | SNDDEV_CAP_VOICE),
	.name = "sec_loopback_tx",
	.copp_id = 1,
	.profile = &loopsmic_profile,
	.channel_mode = 1,
	.default_sample_rate = 48000,
	.pamp_on = msm_snddev_enable_amic_power,
	.pamp_off = msm_snddev_disable_amic_power,
...
static struct platform_device msm_loopsmic_device = {
	.name = "snddev_icodec",
	.dev = { .platform_data = &snddev_loopsmic_data },
};
//loop second mic test end
//yanghai add end
...
#endif
//liuyan modify in 2012-3-13 end
...
//qualcom origin, liuyan modify in 2012-3-13
#if 0 
...
#endif  //liuyan modify in 2012-3-13 end
...
#ifndef CONFIG_OPPO_MODIFY //modified by liuyan in 2012-3-13
