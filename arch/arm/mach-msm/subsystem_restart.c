...
int subsystem_restart(const char *subsys_name)
{
	...
	case RESET_SUBSYS_INDEPENDENT:
		 ...
		 * sequence.
		 */
		 //oppo, add by yben, for modem don't reset when audio reset in COUPLED mode.
		if(restart_level == RESET_SUBSYS_COUPLED && !strcmp(subsys_name,"lpass"))
			data->coupled = 0; 
	...
	case RESET_SOC:
        /* mwalker for modem reset record. */
        if (!strcmp("modem", subsys->name))
            panic("modem");
        else{

		//liuyan 2012.7.23, restart the lpass alone
               data = kzalloc(sizeof(struct restart_thread_data), GFP_KERNEL);
		if (!data) {
		...
		if (IS_ERR(tsk))
			panic("%s: Unable to create thread to restart %s",
				__func__, subsys->name);
		pr_info("liuyan Restrart the dsp ************************* \n");
        	}
		    //panic("subsys-restart: Resetting the SoC - %s crashed.",
			  //  subsys->name);
	       //liuyan 2012.7.23, restart the lpass alone end
		break;
...
static int __init subsys_restart_init(void)
{
	int ret = 0;
       ...
       /*OPPO modify by wjp 2012-07-24 begin*/
	//restart_level = RESET_SOC;
	restart_level = RESET_SUBSYS_COUPLED;
	/*OPPO modify by wjp 2012-07-24 end*/
