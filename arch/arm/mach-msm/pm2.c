...
/* OPPO 2012-07-20 wangjc Delete begin for dell log in resume. */
#if 0
	MSM_PM_DPRINTK(MSM_PM_DEBUG_SUSPEND, KERN_INFO,
		"%s(): return %d\n", __func__, ret);
#endif
...
/*OPPO 2011-10-18 Jiangsm add begin for msm boot mode*/
#define MSM_REBOOT_INTO__PM_FASTBOOT_MODE	0x77665500
#define MSM_REBOOT_INTO__PM_RECOVERY_MODE 	0x77665502
#define MSM_REBOOT_INTO__PM_DOWNLOAD_MODE	0x77665503
#define MSM_REBOOT_INTO__PM_ERASE_MODE			0x776655ef
#define MSM_REBOOT_INTO__PM_OEM_MODE			0x6f656d00
#define MSM_REBOOT_INTO__PM_FACTORY_MODE		0x77665504
#define MSM_REBOOT_INTO__PM_WLAN_MDOE			0x77665505
#define MSM_REBOOT_INTO__PM_RF_MODE			0x77665506
#define MSM_REBOOT_INTO__PM_OTHER_MODE 		0x77665501

/*OPPO 2011-10-18 Jiangsm add end*/
static int msm_reboot_call
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	printk(KERN_INFO songxh msm_reboot_call  "%s for %s\n", __func__, cmd); /* mwalker debug */
/*OPPO 2011-10-18 Jiangsm modify begin for use macros instead of constant number to define reboot mode*/
#if 0	
	...
	if ((code == SYS_RESTART) && _cmd) {
		...
		if (!strcmp(cmd, "fastboot")) {
			restart_reason = MSM_REBOOT_INTO__PM_FASTBOOT_MODE;
		} else if (!strcmp(cmd, "recovery")) {
			restart_reason = MSM_REBOOT_INTO__PM_RECOVERY_MODE;
		} else if (!strcmp(cmd, "eraseflash")) {
			restart_reason = MSM_REBOOT_INTO__PM_ERASE_MODE;
		} else if (!strncmp(cmd, "oem-", 4)) {
			unsigned code = simple_strtoul(cmd + 4, 0, 16) & 0xff;
			restart_reason = MSM_REBOOT_INTO__PM_OEM_MODE | code;
/*OPPO 2011-10-18 Jiangsm add begin for msm reboot into download mode*/
		} else if (!strcmp(cmd, "download") || !strcmp(cmd, "dl")){
			restart_reason = MSM_REBOOT_INTO__PM_DOWNLOAD_MODE;
		} else if (!strcmp(cmd, "factory") || !strcmp(cmd,"ftm")){
			restart_reason = MSM_REBOOT_INTO__PM_FACTORY_MODE;
		} else if (!strcmp(cmd, "radio") || !strcmp(cmd, "rf")){
			restart_reason = MSM_REBOOT_INTO__PM_RF_MODE;
		} else if (!strcmp(cmd, "wlan")){
			restart_reason = MSM_REBOOT_INTO__PM_WLAN_MDOE;
		}
/*OPPO 2011-10-18 Jiangsm add end*/
		/* mwalker port code for set reset reason from m3 */
		else if(!strcmp(cmd, "kernel")) {
			restart_reason = 0x7766550a;
		} else if(!strcmp(cmd, "modem")) {
			restart_reason = 0X7766550b;
		} else if(!strcmp(cmd, "android")) {
			restart_reason = 0x7766550c;
		}
		/* end */
		else {
...
#endif
/*OPPO 2011-10-18 Jiangsm modify  end*/
	return NOTIFY_DONE;
}
