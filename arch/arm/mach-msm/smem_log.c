...
#ifdef  FEATURE_OPPO_MODEM_START_HANDSHAKE
struct vendor1_shared_ram_type
{
   uint32_t handshake;  
   /* TBD: other attribute */
};
...
#ifdef  FEATURE_OPPO_MODEM_START_HANDSHAKE
static struct vendor1_shared_ram_type  __iomem *vendor1_shared_ram = NULL;
#endif
...
static int _smem_log_init(void)
{
   ...
   #ifdef FEATURE_OPPO_MODEM_START_HANDSHAKE
   vendor1_shared_ram = (struct vendor1_shared_ram_type *)smem_alloc(SMEM_ID_VENDOR1, sizeof(struct vendor1_shared_ram_type));
   if(vendor1_shared_ram == NULL )
       pr_info("%s: SMEM_ID_VENDOR1 allocated failed\n", __func__);
   else
       pr_info("%s: SMEM_ID_VENDOR1 allocated successful\n", __func__);
...
static int smem_vendor1_open(struct inode *ip, struct file *fp)
{
    return 0;
}

static int smem_vendor1_release(struct inode *ip, struct file *fp)
{
    return 0;
}

static DEFINE_SPINLOCK(smem_vendor1_lock);

static ssize_t smem_vendor1_read(struct file *fp, char __user *buf,
			size_t count, loff_t *pos)
{
	unsigned long flags;
	int ret;

    if (count < sizeof(struct vendor1_shared_ram_type))
        return -EINVAL;

    if(vendor1_shared_ram == NULL)
        return -1;

    //TODO: remote spinlock. 
	spin_lock_irqsave(&smem_vendor1_lock, flags);
 
    ret = copy_to_user(buf, vendor1_shared_ram,  sizeof(struct vendor1_shared_ram_type));
    if (ret) {
        ret = -EIO;
    }   

    spin_unlock_irqrestore(&smem_vendor1_lock, flags);

	return ret;
}

static ssize_t smem_vendor1_write(struct file *fp, const char __user *buf,
			 size_t count, loff_t *pos)
{
    unsigned long flags;
	int ret;
    struct vendor1_shared_ram_type tmp_buff;
    ...
    ret = copy_from_user(&tmp_buff, buf, sizeof(struct vendor1_shared_ram_type));
    if (ret) 
    {
        printk("ERROR %s:%i tried to write %i, got ret %i",
               __func__, __LINE__,
               sizeof(struct vendor1_shared_ram_type), ret);
    }
    else
    {
        // copy to dest, only handshake copied.
        vendor1_shared_ram->handshake = tmp_buff.handshake;
    }

	wmb();
	spin_unlock_irqrestore(&smem_vendor1_lock, flags);
    
    count = sizeof(struct vendor1_shared_ram_type);
	return count;
}

static const struct file_operations smem_vendor1_fops = {
    .owner = THIS_MODULE,
	.read = smem_vendor1_read,
	.write = smem_vendor1_write,
	.open = smem_vendor1_open,
    .release = smem_vendor1_release,
};
#endif
...
#ifdef FEATURE_OPPO_MODEM_START_HANDSHAKE
static struct miscdevice smem_vendor1_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "smem_vendor1",
	.fops = &smem_vendor1_fops,
};
#endif
...
#ifdef FEATURE_OPPO_MODEM_START_HANDSHAKE
    ret = misc_register(&smem_vendor1_dev);
	if (ret < 0) {
		pr_err("%s: smem_vendor1_dev register failed %d\n", __func__, ret);
		return ret;
	}
#endif
