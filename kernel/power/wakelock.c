...
/* OPPO 2011-04-29 huanggd Add begin*/
int sysfs_get_active_wakelock(char *buf)
{
	...
	list_for_each_entry(lock, &active_wake_locks[WAKE_LOCK_SUSPEND], link) {
		if (!(lock->flags & WAKE_LOCK_AUTO_EXPIRE)) // non-autoexpired wakelock
...
/* OPPO 2011-04-29 huanggd Add end */
...
//add by huanghao 2011.11.25 for wakelock debug
static void wakelock_dbg_wake_locks(unsigned long data);
static DEFINE_TIMER(wakelock_dbg_timer, wakelock_dbg_wake_locks, 0, 0);
char wake_buf[1024] = {0};
static void wakelock_dbg_wake_locks(unsigned long data)
{
	
	memset(wake_buf, 0, 1024);
	sysfs_get_active_wakelock(wake_buf);
	printk(KERN_ERR "wakelock active_wakelock = %s\n", wake_buf);
	mod_timer(&wakelock_dbg_timer, jiffies +20 * HZ);
}
//end by huanghao 2011.11.25 for wakelock debug
...
static void suspend(struct work_struct *work)
{
	...
	//add by huanghao 2011.11.25 for wakelock debug
	del_timer(&wakelock_dbg_timer);
	//endby huanghao 2011.11.25 for wakelock debug
...
/* OPPO 2011-04-15 huanggd Add begin for avoid block in early_suspend() when large file is reading/writing */
struct wake_lock sys_sync_wake_lock;
struct workqueue_struct *sys_sync_work_queue;
/* OPPO 2011-04-15 huanggd Add end */
